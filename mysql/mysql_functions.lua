...
function setBizDataSQL ( )
	...
	if result then
		for i=1, #result do
			...
			-- war: "if biz == ..." ( undefinierte globale Variable, also nie wahr )
			-- und darin "bizArray[i]" ( numerischer Index, obwohl bizArray ueber den
			-- Biz-Namen indiziert wird ). Der Pizzaladen blieb dadurch kaeuflich,
			-- obwohl biz.lua ihn beim Kassen-Speichern bewusst ueberspringt.
			if bizname == "WellStackedPizza" then
				-- Steht in der Datenbank noch ein Spieler als Besitzer, wird der
				-- Eintrag geloest. Sonst koennte dieser Spieler den Laden per
				-- /sellbiz zum Mafia-Festpreis von 99999999 verkaufen.
				local altbesitzerUID = tonumber ( dsatz["UID"] ) or 0
				if altbesitzerUID > 0 then
					dbExec ( handler, "UPDATE ?? SET ??=? WHERE ??=?", "biz", "UID", 0, "ID", tonumber ( dsatz["ID"] ) )
					dbExec ( handler, "UPDATE ?? SET ??=? WHERE ??=?", "userdata", "Bizschluessel", 0, "UID", altbesitzerUID )
					outputDebugString ( "[setBizDataSQL] WellStackedPizza gehoert der Mafia - Besitzereintrag von UID "..altbesitzerUID.." entfernt." )
				end
				bizArray[bizname]["inhaber"] = "Mafia"
				bizArray[bizname]["preis"] = 99999999
			end
		end
		return true
	end
...
function loadPlayingTimeForSleeplessAchiev ( player, pname )
	...
	MtxSetElementData ( player, "timePlayedToday", time )
end
...
function bonusLoad ( player )
	...
	if dsatz and dsatz[1] then
		...
		MtxSetElementData ( player, "lungenvol", Lungenvolumen )
		if Lungenvolumen ~= "none" then
			setPedStat ( player, 225, 500 )
		end
		local Muskeln = dsatz["Muskeln"]
		MtxSetElementData ( player, "muscle", Muskeln )
		if Muskeln ~= "none" then
			setPedStat ( player, 23, 500 )
		end
		local Kondition = dsatz["Kondition"]
		MtxSetElementData ( player, "stamina", Kondition )
		if Kondition ~= "none" then
			setPedStat ( player, 22, 500 )
		end
		local PistolenSkill = dsatz["PistolenSkill"]
		MtxSetElementData ( player, "pistolskill", PistolenSkill )
		if PistolenSkill ~= "none" then
			setPedStat ( player, 69, 900 )
			setPedStat ( player, 70, 999 )
		end
		local DeagleSkill = dsatz["DeagleSkill"]
		MtxSetElementData ( player, "deagleskill", DeagleSkill )
		if DeagleSkill ~= "none" then
			setPedStat ( player, 71, 999 )
		end
		local ShotgunSkill = dsatz["ShotgunSkill"]
		MtxSetElementData ( player, "shotgunskill", ShotgunSkill )
		if ShotgunSkill ~= "none" then
			setPedStat ( player, 72, 999 )
			setPedStat ( player, 74, 999 )
		end
		local AssaultSkill = dsatz["AssaultSkill"]
		MtxSetElementData ( player, "assaultskill", AssaultSkill )
		if AssaultSkill ~= "none" then
			setPedStat ( player, 77, 999 )
			setPedStat ( player, 78, 999 )
		end
		local MP5Skill = dsatz["MP5Skills"]
		MtxSetElementData ( player, "mp5skill", MP5Skill )
		if MP5Skill ~= "none" then
			setPedStat ( player, 76, 999 )
		end
		
		MtxSetElementData ( player, "boxen", dsatz["Boxen"] )
		MtxSetElementData ( player, "kungfu", dsatz["KungFu"] )
		MtxSetElementData ( player, "streetfighting", dsatz["Streetfighting"] )
		setPedFightingStyle ( player, tonumber ( dsatz["CurStyle"] ) )
		MtxSetElementData ( player, "vortex", dsatz["Vortex"] )
		MtxSetElementData ( player, "quad", dsatz["Quad"] )
		MtxSetElementData ( player, "skimmer", tonumber ( dsatz["Skimmer"] ) )
		MtxSetElementData ( player, "romero", tonumber ( dsatz["Leichenwagen"] ) )
		MtxSetElementData ( player, "carslotupgrade", dsatz["CarslotUpgrades"] )
		MtxSetElementData ( player, "bonusskin1", dsatz["BonusSkin1"] )
		MtxSetElementData ( player, "golfcart", dsatz["Caddy"] == "1" )
		MtxSetElementData ( player, "fglass", dsatz["fglass"] == "1" )
		MtxSetElementData ( player, "doubleSMG", dsatz["uzi"] == "1" )
...
function packageLoad ( player )
	...
	if dsatz and dsatz[1] then
		...
		for i = 1, #packages do
			...
			MtxSetElementData ( player, "package"..i, paket )
			if paket == 1 then
				number = number + 1
...
function mySQLMailCheck ( array )
	...
	-- war: "#amount" - amount ist eine Zahl, das warf beim Serverstart
	-- "attempt to get length of a number" und brach die Mail-Bereinigung ab.
	for i=1, amount do
...
function loadPlayerStatisticsMySQL ( player )
	...
	if result and result[1] then
		...
		MtxSetElementData ( player, "Eingeloggt", MtxGetElementData ( player, "Eingeloggt" ) + 1 )
		local realtime = getRealTime()
		local weekday = realtime.weekday
		if not wasOnlineToday[pname] then
			MtxSetElementData ( player, weekdayName[weekday].."Spielzeit", realtime.timestamp )
			wasOnlineToday[pname] = true
		else
			local minuten = MtxGetElementData ( player, weekdayName[weekday].."Spielzeit" )
			MtxSetElementData ( player, weekdayName[weekday].."Spielzeit", realtime.timestamp - minuten * 60 )
		end
		return true
	end
	return false
	...
	if MtxGetElementData ( player, "loggedin" ) == 1 then
		...
		if gangAreaUnderAttack then
			if playerData[getPlayerName(player)] then
				MtxSetElementData ( player, "AnzahlGangwarsVerloren", MtxGetElementData ( player, "AnzahlGangwarsVerloren" ) + 1 )
			end
		end
		local realtime = getRealTime()
		local weekday = realtime.weekday
		local newtime = math.floor ( ( realtime.timestamp - ( MtxGetElementData ( player, weekdayName[weekday].."Spielzeit" ) or 0 ) ) / 60 )
		MtxSetElementData ( player, weekdayName[weekday].."Spielzeit", newtime )
		if weekday == 1 and not wasOnlineToday[pname] then
			local wochenzeit = 0
			-- war: weekdayName[weekday] in der Schleife - dadurch wurde 7x derselbe
			-- Tag addiert statt aller sieben. weekdayName ist ab 0 indiziert.
			for i=0, 6 do
				wochenzeit = wochenzeit + ( MtxGetElementData ( player, weekdayName[i].."Spielzeit" ) or 0 )
			end
	...
	if result and result[1] then
		for i=1, #result do
			...
			if Besitzer then
				...
				MtxSetElementData ( vehicle, "color", dsatz["Farbe"] )
				MtxSetElementData ( vehicle, "lcolor", dsatz["Lights"] )
				...
				MtxSetElementData ( vehicle, "distance", tonumber ( dsatz["Distance"] ) )
				MtxSetElementData ( vehicle, "stuning", dsatz["STuning"] )
				MtxSetElementData ( vehicle, "spezcolor", dsatz["spezcolor"] )
				setVehiclePlateText( vehicle, dsatz["plate"] )
				MtxSetElementData ( vehicle, "rcVehicle", tonumber ( dsatz["rc"] ) )
				MtxSetElementData ( vehicle, "sportmotor", tonumber (dsatz["Sportmotor"]) )
				MtxSetElementData ( vehicle, "bremse", tonumber (dsatz["Bremse"] ) )
				MtxSetElementData ( vehicle, "antrieb", dsatz["Antrieb"] )
				MtxSetElementData ( vehicle, "totalschaden", dsatz["totalschaden"] )
				MtxSetElementData ( vehicle, "Beschlagnahmt", dsatz["Beschlagnahmt"] )
				MtxSetElementData ( vehicle, "KeyTarget", tonumber(dsatz["KeyTarget"] ) or false )
				setVehicleLocked ( vehicle, true )
				MtxSetElementData ( vehicle, "fuelstate", tonumber ( dsatz["Benzin"] ) )
				...
				if MtxGetElementData(vehicle, "Beschlagnahmt") == 1 then
					setElementPosition(vehicle, -2068.6342773438,-134.87577819824,34.919010162354)
					setElementDimension(vehicle, 4515)
					setElementRotation (vehicle,0,0,180)
					setElementFrozen( vehicle, true)
					setVehicleDamageProof( vehicle, true )
					setVehicleEngineState ( vehicle, false )
					MtxSetElementData ( vehicle, "engine", false )
				end
