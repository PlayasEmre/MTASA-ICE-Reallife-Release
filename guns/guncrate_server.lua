...
function exchangeWeaponsWithBox_func ( player, slot, action )
	...
	if player == client then
		...
		if isOnDuty(player) then
			...
			if action == "ex" then
				local slot1 = MtxGetElementData ( player, "gunboxa" )
				local slot2 = MtxGetElementData ( player, "gunboxb" )
				local slot3 = MtxGetElementData ( player, "gunboxc" )
			...
			elseif action == "in" then
				if slot == 1 then
					MtxSetElementData ( player, "gunboxa", curgun.."|"..curammo )
				elseif slot == 2 then
					MtxSetElementData ( player, "gunboxb", curgun.."|"..curammo )
				else
					MtxSetElementData ( player, "gunboxc", curgun.."|"..curammo )
				end
