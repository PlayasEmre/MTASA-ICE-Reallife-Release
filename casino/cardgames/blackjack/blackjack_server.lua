...
function startBlackJack_func ( amount )
	...
	if player == source then
		if not MtxGetElementData ( player, "blackJackStarted" ) then
			MtxSetElementData ( player, "blackJackStarted", true )
			...
			if amount <= chips then
				if amount <= calcPlayerMaxGambleBet ( player, 1 ) then
					if amount > 0 then
						MtxSetElementData ( player, "curBlackJackBet", amount )
					...
					infobox_start_func ( "Dafuer ist dein\nSpielerskill nicht\nhoch genug!\nMaximaleinsatz:\n"..calcPlayerMaxGambleBet ( player, 1 ).." "..Tables.waehrung.."", 5000, 125, 0, 0 )
				end
			end
		else
...
function endBlackJackCurGame ( player, lost, blackJack )
	...
	if MtxGetElementData ( player, "blackJackStarted" ) then
		MtxSetElementData ( player, "blackJackStarted", false )
		local r, g, b, text = 0, 0, 0, ""
		local bet = MtxGetElementData ( player, "curBlackJackBet" )
		
		if not lost then
			casinoAchievCheck ( player, bet )
			g = 150
			if blackJack then
				text = "Du gewinnst "..(bet*2).." "..Tables.waehrung..""
				increasePlayerGambleSkillLevel ( player, bet*2 )
			else
				text = "Du gewinnst "..bet.." "..Tables.waehrung..""
				increasePlayerGambleSkillLevel ( player, bet )
			end
			if not blackJack then
				MtxSetElementData ( player, "casinoChips", MtxGetElementData ( player, "casinoChips" ) + bet * 2 )
			else
				MtxSetElementData ( player, "casinoChips", MtxGetElementData ( player, "casinoChips" ) + bet * 3 )
			end
		else
			casinoAchievCheck ( player, -1 * bet )
			text = "Du verlierst "..bet.." "..Tables.waehrung..""
			r = 150
		end
...
function blackJackDouble_func ()
	...
	if i == 6 then
		...
		local chips = MtxGetElementData ( player, "casinoChips" )
		if chips >= bet then
			MtxSetElementData ( player, "casinoChips", chips - bet )
			MtxSetElementData ( player, "curBlackJackBet", bet * 2 )
			local player = client
			local i = blackJackCards[player]["playerCardCount"] + 1
			blackJackCards[player]["playerCardCount"] = i
			blackJackCards[player]["player"][i] = getBlackJackCard ( player )
