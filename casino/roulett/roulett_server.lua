...
function startRoulett_func ( string, count )
	...
	local player = client
	local betMoney = 0
	local playerChips = MtxGetElementData ( player, "casinoChips" )
	...
	if betMoney <= playerChips then
		if betMoney <= calcPlayerMaxGambleBet ( player, 0 ) then
			MtxSetElementData ( player, "casinoChips", playerChips - betMoney )
			...
			local diff = MtxGetElementData ( player, "casinoChips" ) - playerChips
			...
			triggerClientEvent ( player, "showRoulettWinningNumber", player, roulettWinningNumber, MtxGetElementData ( player, "casinoChips" ) - playerChips )
		else
			infobox ( player, "Dafuer ist dein\nSpielerskill nicht\nhoch genug!\nMaximaleinsatz:\n"..calcPlayerMaxGambleBet ( player, 0 ).." "..Tables.waehrung.."", 5000, 125, 0, 0 )
...
function roulettWin ( multiply, amount, player )
	...
	MtxSetElementData ( player, "casinoChips", MtxGetElementData ( player, "casinoChips" ) + multiply * amount )
end