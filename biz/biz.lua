...
function setBizData ()
	...
	bizkeys = { [1]="PaynSprayWangcars", [2]="PaynSprayJuniper",[3]="WellStackedPizza", [4]="MistysBar", [5]="Visage", [6]="VankHoff", [7]="TankstelleNord", [8]="TankstelleSued", [9]="TankstellePine", [10]="PaynSprayLV", [11]="PaynSprayLS" }
end
...
function onBizPickupHit ( hit )
	...
	elseif bizIcons[source] then
		...
		outputChatBox ( bizname..", gehört: "..besitzer.." - Preis: "..bizprice.." "..Tables.waehrung.."", hit, 125, 0, 0 )
	...
	elseif OttosAutosIcon == source then
		outputChatBox ( "Tippe /sellcarto [Name] [Eigener Slot] [Slot beim neuen Besitzer], um das Auto an jemanden zu geben.", hit, 200, 200, 0 )
	end
end
...
function buybiz_func ( player, cmd, typ )
	...
	local cash = 0
	if typ == "bar" then
		cash = MtxGetElementData ( player, "money" )
		hatgeld = true
	elseif typ == "bank" then
		cash = MtxGetElementData ( player, "bankmoney" )		
		hatgeld = true	
		if not cash then
			hatgeld = false
		end
		...
		if biz then
			...
			if bizprice <= cash then
				if bizArray[biz]["inhaber"] == "none" then
					if MtxGetElementData ( player, "bizkey" ) == 0 then
						...
						if typ == "bar" then
							...
							MtxSetElementData ( player, "bankmoney", MtxGetElementData ( player, "bankmoney" ) - bizprice )
						end
...
function sellbiz_func ( player )
	if MtxGetElementData ( player, "bizkey" ) == 0 then
		...
		local key = MtxGetElementData ( player, "bizkey" )
...
function bizhelp_func ( player )
	local bizkey = tonumber ( MtxGetElementData ( player, "bizkey" ) )
	if bizkey == 0 then
		outputChatBox ( "Du besitzt kein Geschaeft!", player, 125, 0, 0 )
	else
		local biz = bizkeys[bizkey]
		local bizname = bizArray[biz]["name"]
		outputChatBox ( "Dein Geschäft: "..bizname..", Kasse: "..bizArray[biz]["kasse"].." "..Tables.waehrung.."", player, 200, 200, 15 )
		...
		if amount then
			...
			if amount then
				...
				if curmoney >= amount then
					...
					infobox ( player, amount..""..Tables.waehrung.."\nrausgenommen", 4000, 0, 200, 0 )
				else
