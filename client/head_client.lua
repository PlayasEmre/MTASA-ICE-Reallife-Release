...
-- Bildschirmmitte einmal ausrechnen statt 10x pro Sekunde.
local screenMidX, screenMidY = screenwidth / 2, screenheight / 2

function adjustPlayerHead ()

	if getCameraTarget () then
		-- war global ( x, y, z ): dieser Timer hat 10x pro Sekunde drei globale
		-- Variablen ueberschrieben - unnoetige Schreibzugriffe auf _G und ein
		-- Risiko fuer jedes andere Client-Skript, das x/y/z benutzt.
		local x, y, z = getWorldFromScreenPosition ( screenMidX, screenMidY, 50 )
		if x then
...
function lookGlass ()
	...
	if not spyGoglesActive and not getElementClicked () and vioClientGetElementData ( "fglass" ) then
		...
		gImage["spyGogles"] = guiCreateStaticImage ( 0, 0, 1, 1, ":"..getResourceName(getThisResource()).."/images/lookout.png", true )
		gImage["spyGoglesRedDot"] = guiCreateStaticImage ( screenwidth / 2 - 2, screenheight / 2 - 2, 4, 4, ":"..getResourceName(getThisResource()).."/images/reddot.png", false )
	end
end
