...
function weather_func ()
	...
	if weather == 0 then
		waves = 0
	elseif weather >= 1 and weather <= 7 then						--- leicht Bewölkt, kein Regen
...
function changeWeather ( weather, waves )
	...
	-- Waehrend Halloween bleibt es duester ( events/halloween_server.lua setzt das
	-- Wetter einmal ). Der Kreislauf laeuft weiter, damit das Zufallswetter nach
	-- dem Event ohne Neustart wieder greift.
...
function setWaterLVLLower ( water )
	...
	if isElement ( water ) then
		...
		if waterlevel <= 0 then
			...
			if isTimer ( setWaterLVLLowerTimer ) then killTimer ( setWaterLVLLowerTimer ) end
		else
			setWaterLevel ( water, waterlevel )
			setWaterLevel ( waterlevel )
		end
