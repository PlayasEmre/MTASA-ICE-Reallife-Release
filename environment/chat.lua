...
function chatAble ( player )
	...
	if MtxGetElementData ( player, "loggedin" ) == 1 and not isPedDead ( player ) then
		return true
	end
	return false
end
...
function sendMessageToNearbyPlayers ( message, messageType )
	...
	if messageType == 0 and chatAble ( source ) then
		if tonumber(MtxGetElementData ( source, "muted" )) == 0 then
			local posX, posY, posZ = getElementPosition( source )
			local chatSphere = createColSphere( posX, posY, posZ, 10 )
			local nearbyPlayers = getElementsWithinColShape( chatSphere, "player" )
			destroyElement( chatSphere )
			if MtxGetElementData ( source, "isLive" ) then
				...
				for i=1, #nearbyPlayers do
					...
					if getElementDimension ( source ) == getElementDimension ( nearbyPlayer ) then
						if MtxGetElementData ( source, "call" ) then
							if isElement ( nearbyPlayer ) then
								outputChatBox ( pname.. " (Handy) sagt: " ..message, nearbyPlayer, rgb, rgb, rgb )
							end
						else
							if MtxGetElementData ( source, "callswithpolice" ) == true then
								local taeter = getPlayerFromName ( message )
								if taeter then
									outputChatBox ( "Wir werden der Sache nachgehen.", source, 0, 0, 200 )
									crime = "none"
									if MtxGetElementData ( taeter, "lastcrime" ) == "mord" then
										crime = "Mord"
									elseif MtxGetElementData ( taeter, "lastcrime" ) == "drogen" then
										crime = "Drogennehmen"
									elseif MtxGetElementData ( taeter, "lastcrime" ) == "drugdealing" then
										crime = "Drogenhandel"
									elseif MtxGetElementData ( taeter, "lastcrime" ) == "mats" then
										crime = "Waffenhandel"
									elseif MtxGetElementData ( taeter, "lastcrime" ) == "violance" then
									...
									if crime ~= "none" then
										...
										if MtxGetElementData ( taeter, "wanteds" ) <= 5 then
											MtxSetElementData ( taeter, "wanteds", MtxGetElementData ( taeter, "wanteds" ) + 1 )
											setPlayerWantedLevel ( taeter, MtxGetElementData ( taeter, "wanteds" ) )
										end
										MtxSetElementData ( taeter, "lastcrime", "none" )
									end
								else
									outputChatBox ( "Es tut uns Leid Sir, der Täter ist uns unbekannt.", source, 0, 0, 200 )
								end
								MtxSetElementData ( source, "callswithpolice", false )
							elseif MtxGetElementData ( source, "callswithmedic" ) == true then
								MtxSetElementData ( source, "callswithmedic", false )
								...
								MtxSetElementData ( source, "needhelpmedic", true )
							...
							elseif MtxGetElementData ( source, "callswithmechaniker" ) == true then
								MtxSetElementData ( source, "callswithmechaniker", false )
								...
								MtxSetElementData ( source, "needhelpmechaniker", true )
			...
			if isElement ( MtxGetElementData(source,"callswith") ) then
				local target = MtxGetElementData(source,"callswith")
	...
	if target and findPlayerByName(target) then
		...
		if isEmergencyOnDuty ( player ) then
			if isMedic ( player ) then
				if MtxGetElementData ( targetpl, "needhelpmedic" ) then
					...
					MtxSetElementData ( targetpl, "needhelpmedic", false )
			...
			elseif isMechaniker ( player ) then
				if MtxGetElementData ( targetpl, "needhelpmechaniker" ) then
					...
					MtxSetElementData ( targetpl, "needhelpmechaniker", false )
