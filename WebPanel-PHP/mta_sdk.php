...
class mta
{
	...
	/*
		PHP 8: Methoden mit dem Klassennamen sind KEINE Konstruktoren mehr.
		Vorher wurden host/port/user/pass deshalb nie gesetzt und jeder
		Aufruf endete mit "Could not connect to :".
	*/
	...
	public function getResource ( $resourceName )
	{
		...
		{
			// getName ist eine Methode - ohne Klammern war der Vergleich immer falsch.
	...
	public function callFunction( $resourceName, $function, $args )
	{
		...
		// Das urspruengliche "echo $json_output;" hat die Anfrage in die Seite geschrieben.
	...
	public static function convertToObjects( $item )
	{
		...
		{	
			...
			{
				/*
					Hier stand $this->getResource(...) - in einer statischen
					Methode ist $this aber nicht verfuegbar (Fatal Error).
					Der Resource-Name wird daher unveraendert zurueckgegeben.
				*/
				$item = substr( $item, 3 );
			}
		}
	...
	function do_post_request( $host, $port, $path, $json_data )
	{
		...
		else
		{
			...
			$tmp = explode( "\r\n\r\n", $response, 2 );
			$headers = $tmp[0];
			// Antwort ohne Body: $tmp[1] existiert dann nicht (PHP-8-Warnung).
			$response = ((isset($tmp[1])) ? trim($tmp[1]) : '');
...
class Element
{
	...
	// PHP 8: "function Element(...)" war kein Konstruktor mehr -> $id blieb leer.
...
class Resource
{
	...
	// PHP 8: "function Resource(...)" war kein Konstruktor mehr -> $server blieb null.
