...
/*
	Hilfsfunktionen fuer die ICE-Anpassung.
	Die ICE-Datenbank hat einige Tabellen/Spalten nicht, die das Panel
	urspruenglich vorausgesetzt hat. Damit keine Seite mit einem Fatal Error
	abbricht, wird vorher geprueft ob die Tabelle bzw. Spalte existiert.
*/
/*
	Adminlevel-Nummer -> Rangname.
	Muss mit dem Script uebereinstimmen (client/nametag_client.lua Z. 161-180,
	admin/admincmds.lua "adminLevels") - sonst zeigt das Panel einen anderen
	Rang an als im Spiel ueber dem Spielerkopf steht.
*/
function cpAdminRangName($level) {
	$raenge = array(
		1 => "Supporter",
		2 => "Moderator",
		3 => "Administrator",
	...
	$level = (int)$level;
	if ($level <= 0) { return "-"; }
	return (isset($raenge[$level])) ? $raenge[$level] : "Admin (Level ".$level.")";
}

function cpTableExists($con, $table) {
	static $cache = array();
	if (isset($cache[$table])) { return $cache[$table]; }
	$res = @$con->query("SHOW TABLES LIKE '".$con->escape_string($table)."'");
	$cache[$table] = ($res && $res->num_rows > 0);
	return $cache[$table];
}

function cpColumnExists($con, $table, $column) {
	static $cache = array();
	$key = $table.'.'.$column;
	if (isset($cache[$key])) { return $cache[$key]; }
	if (!cpTableExists($con, $table)) { $cache[$key] = false; return false; }
	$res = @$con->query("SHOW COLUMNS FROM `".str_replace('`', '', $table)."` LIKE '".$con->escape_string($column)."'");
	$cache[$key] = ($res && $res->num_rows > 0);
	return $cache[$key];
}

/*
	Wandelt eine Punktliste in sicheres JavaScript um.

	$punkte: array( array("x" => .., "y" => .., "titel" => ..), ... )
	Rueckgabe: z.B.  [[1234,-567,"Mein Haus"],[...]]

	Warum json_encode und nicht selbst zusammenbauen:
	Bei einem selbst gebauten String koennte ein Text mit Anfuehrungszeichen
	oder </script> aus dem JavaScript "ausbrechen" und eigenen Code einbringen.
	json_encode mit diesen Flags macht daraus harmlose Zeichenfolgen.
*/
function cpPunkteJs($punkte) {
	$liste = array();
	foreach ($punkte as $p) {
	...
	$json = json_encode($liste, $flags);
	return (($json === false) ? '[]' : $json);
}

/*
	Ist die PHP-Erweiterung GD vorhanden?
	Sie wird nur noch von ajax/map.php gebraucht - die Karte im Panel
	funktioniert inzwischen auch ohne GD (siehe cpKarte).
*/
function cpGdVorhanden() {
	return function_exists('imagecreatefromjpeg');
}

/*
	Karte mit Markierungen - OHNE die PHP-Erweiterung GD.

	Vorher wurde das Bild von ajax/map.php erzeugt. Fehlt GD (in XAMPP ist
	"extension=gd" oft noch auskommentiert), kam statt eines Bildes Text
	zurueck und im Browser war einfach nichts zu sehen.

	Jetzt wird die Karte direkt als Bild eingebunden und die Markierungen
	werden per CSS darueber gelegt. Das laeuft immer.

	$punkte: array( array("x" => 1234.5, "y" => -567.8, "titel" => "Mein Haus"), ... )
*/
function cpKarte($id, $punkte, $mitNummern = false) {
	$daten = array();
	foreach ($punkte as $p) {
		if (!isset($p['x']) || !isset($p['y'])) { continue; }

		// GTA-SA-Welt: -3000 bis +3000  ->  Prozent im Bild
		$links = (((float)$p['x'] + 3000) / 6000) * 100;
		$oben  = ((3000 - (float)$p['y']) / 6000) * 100;

		// Punkte ausserhalb der Karte ueberspringen (z.B. alte Platzhalter).
		if ($links < 0 || $links > 100 || $oben < 0 || $oben > 100) { continue; }

		$daten[] = array(
			"l" => round($links, 3),
			"o" => round($oben, 3),
			"t" => ((isset($p['titel'])) ? $p['titel'] : '')
		);
	}

	echo '<div id="'.htmlspecialchars($id).'" class="cp-karte" style="position: relative; width: 100%; line-height: 0;">
		<img src="assets/img/map.jpg" alt="Karte von San Andreas" style="width: 100%; display: block;" />
		<div class="cp-marker" style="position: absolute; left: 0; top: 0; right: 0; bottom: 0;">';

	$nr = 0;
	foreach ($daten as $d) {
		$nr++;
		$beschriftung = (($mitNummern) ? '<span style="position: absolute; left: 14px; top: -4px; color: #fff; font-size: 12px; font-weight: bold; text-shadow: 0 0 3px #000, 0 0 3px #000; line-height: 1;">'.$nr.'</span>' : '');

		echo '<div style="position: absolute; left: '.$d['l'].'%; top: '.$d['o'].'%; width: 0; height: 0;" title="'.htmlspecialchars($d['t']).'">
	...
	if (count($daten) == 0 && count($punkte) > 0) {
		echo '<p class="text-muted"><small>Für diese Position liegen keine gültigen Koordinaten in der Datenbank.</small></p>';
	}
}

/*
	Sind die Live-Adminfunktionen wirklich benutzbar?

	Die Lua-Seite liefert ICE-Reallife/webpanel/webpanel_server.lua (in der
	meta.xml mit http="true" exportiert). Zusaetzlich braucht das Panel einen
	echten MTA-Account - solange dort noch die Beispielwerte stehen, wuerde
	jeder Aufruf mit "Access Denied" zurueckkommen.
*/
function cpMtaBereit() {
	if (!MTA_WEB_FUNCTIONS) { return false; }
	if (MTA_USER == '' || MTA_PASS == '') { return false; }
	if (MTA_USER == 'User' || MTA_PASS == 'Passwort') { return false; }
	return true;
}

// Erklaerung, was fuer die Live-Adminfunktionen noch fehlt.
function cpMtaHinweis() {
	if (!MTA_WEB_FUNCTIONS) {
		return 'Die Live-Adminfunktionen sind in <code>cfg.php</code> abgeschaltet ('
			.'<code>MTA_WEB_FUNCTIONS</code> auf <code>true</code> setzen).';
	}
	return 'In <code>cfg.php</code> stehen bei <code>MTA_USER</code> / <code>MTA_PASS</code> noch die Beispielwerte.<br>
	Dort einen MTA-Account eintragen, der die Funktionen der Resource aufrufen darf.
	Ausserdem müssen in der <code>mtaserver.conf</code> bei <code>http_dos_exclude</code> und
	<code>auth_serial_http_ip_exceptions</code> die IP des Webservers stehen (lokal: <code>127.0.0.1</code>).';
}

/*
	Ruft eine Funktion im MTA-Server auf und faengt alle Fehler ab.

	Das MTA-SDK wirft bei Verbindungsproblemen oder falschen Zugangsdaten
	eine Exception - ohne try/catch waere das ein Fatal Error mitten in der
	Seite bzw. in der AJAX-Antwort.

	Rueckgabe: array("ok" => bool, "wert" => string, "fehler" => string)
*/
function cpMtaCall($funktion, $args = array()) {
	if (!cpMtaBereit()) {
		return array("ok" => false, "wert" => "", "fehler" => "Die Live-Adminfunktionen sind nicht eingerichtet.");
	}

	/*
		Passwort für die HTTP-Anmeldung.

		Ist in cfg.php ein MTA_HTTP_CODE eingetragen (7 Ziffern aus dem
		Konsolenbefehl "authserial <account> httppass"), wird er hinten an das
		Passwort angehängt. So verlangt es MTAs Serial-Schutz.
	*/
	$httpPass = MTA_PASS;
	if (defined('MTA_HTTP_CODE') && MTA_HTTP_CODE != '') {
		$httpPass = MTA_PASS.MTA_HTTP_CODE;
	}

	try {
		ob_start();
		$mtaServer = new mta(MTA_IP, MTA_HTTP_PORT, MTA_USER, $httpPass);
		$resource = $mtaServer->getResource(MTA_RESOURCE_NAME);
		$antwort = call_user_func_array(array($resource, 'call'), array_merge(array($funktion), $args));
		ob_end_clean();
	} catch (Exception $e) {
		...
		/*
			Die Meldungen des MTA-SDK sind fuer sich genommen kryptisch.
			Hier wird dazugeschrieben, was konkret zu tun ist.
		*/
		if (stripos($meldung, 'Access Denied') !== false || stripos($meldung, 'authentication') !== false) {
			return array("ok" => false, "wert" => "", "fehler" =>
				'<b>Der MTA-Server lehnt die Anmeldung ab.</b><br>
				Schau ins schwarze Serverfenster - dort steht, welcher der beiden Fälle es ist:<br><br>

				<b>Fall 1: „not associated with authorized serial"</b><br>
				MTA schützt Accounts, die in einer ACL-Gruppe aus
				<code>auth_serial_groups</code> stehen (Standard: <code>Admin</code>).
				Bei denen ist eine HTTP-Anmeldung nur von einer IP erlaubt, die der
				Accountinhaber vorher ingame benutzt hat.
				Es gibt <b>vier</b> Wege (Quelle: wiki.multitheftauto.com/wiki/Authorized_Serial_Account_Protection):<br>

				<b>a)</b> IP freigeben in
				<code>...\\server\\mods\\deathmatch\\mtaserver.conf</code>
				(mehrere IPs mit Komma):<br>
				<code>&lt;auth_serial_http_ip_exceptions&gt;127.0.0.1&lt;/auth_serial_http_ip_exceptions&gt;</code><br>

				<b>b)</b> Zugangscode holen — in der Server-Konsole:<br>
				<code>authserial '.htmlspecialchars(MTA_USER).' httppass</code><br>
				Der 7-stellige Code wird <b>hinten an das Passwort angehängt</b>. Steht das
				Passwort z.B. auf <code>geheim</code> und der Code ist <code>1234567</code>,
				dann in der <code>cfg.php</code> bei <code>MTA_PASS</code>
				<code>geheim1234567</code> eintragen.<br>

				<b>c)</b> Mit dem Account einmal <b>ingame einloggen</b> — danach ist die IP
				freigegeben.<br>

				<b>d)</b> Einen Account benutzen, der <b>nicht</b> in der Gruppe
				<code>Admin</code> ist (siehe Fall 2) — dann greift der Schutz gar nicht.<br><br>

				Weitere Befehle: <code>authserial '.htmlspecialchars(MTA_USER).' list</code>
				zeigt die freigegebenen Serials,
				<code>authserial '.htmlspecialchars(MTA_USER).'</code> gibt die letzte frei.<br>
				Stolpersteine: die Zeile in der Konfiguration steht noch <b>innerhalb eines
				&lt;!-- Kommentars --&gt;</b>, oder der Server wurde danach
				<b>nicht neu gestartet</b>.<br><br>

				<b>Fall 2: „valid username and password"</b><br>
				<code>MTA_USER</code> / <code>MTA_PASS</code> sind <b>kein</b> Spiel-Account aus der
				Datenbank, sondern ein eigener <b>MTA-Serveraccount</b>:<br>
				<code>addaccount webpanel DeinPasswort</code> in der Server-Konsole.<br><br>

				<b>Wichtig:</b> Den Account <b>nicht</b> in die Gruppe <code>Admin</code> setzen!
				In der <code>mtaserver.conf</code> steht <code>auth_serial_groups = Admin</code> —
				Accounts dieser Gruppe brauchen eine Serial-Freigabe, und genau daran
				scheitert Fall 1. Nimm stattdessen eine eigene Gruppe in der <code>acl.xml</code>:<br>
				<code>&lt;acl name="WebPanel"&gt;&lt;right name="general.http" access="true"&gt;&lt;/right&gt;
				&lt;right name="resource.'.htmlspecialchars(MTA_RESOURCE_NAME).'.http" access="true"&gt;&lt;/right&gt;&lt;/acl&gt;</code><br>
				<code>&lt;group name="WebPanel"&gt;&lt;acl name="WebPanel"&gt;&lt;/acl&gt;
				&lt;object name="user.webpanel"&gt;&lt;/object&gt;
				&lt;object name="resource.'.htmlspecialchars(MTA_RESOURCE_NAME).'"&gt;&lt;/object&gt;&lt;/group&gt;</code>');
		}

		if (stripos($meldung, 'Could not connect') !== false) {
			return array("ok" => false, "wert" => "", "fehler" =>
				'<b>Keine Verbindung zum MTA-Server.</b><br>
				Läuft der Server? Stimmen <code>MTA_IP</code> ('.htmlspecialchars(MTA_IP).') und
				<code>MTA_HTTP_PORT</code> ('.htmlspecialchars(MTA_HTTP_PORT).') in der <code>cfg.php</code>?
				Der Wert muss zu <code>httpport</code> in der <code>mtaserver.conf</code> passen.');
		}

		if (stripos($meldung, 'resource exists') !== false || stripos($meldung, 'spelled correctly') !== false
			|| stripos($meldung, 'not found') !== false) {

			return array("ok" => false, "wert" => "", "fehler" =>
				'<b>Der MTA-Server kennt die aufgerufene Funktion nicht („Not Found").</b><br>
				Die Anmeldung hat also geklappt — es fehlt nur die Freigabe der Funktionen.<br><br>

				<b>Das fehlt fast immer:</b> die <code>&lt;exports&gt;</code>-Zeilen in der
				<code>meta.xml</code> der Resource <code>'.htmlspecialchars(MTA_RESOURCE_NAME).'</code>.
				Ohne <code>http="true"</code> darf das Panel eine Funktion nicht aufrufen.<br><br>

				<b>So prüfst Du es:</b> öffne die <code>meta.xml</code> auf dem Server und suche nach
				<code>listAllPlayers</code>. Findest Du nichts, fehlt der ganze Block —
				er steht in der Datei <code>ICE-Reallife\\meta.xml</code> am Ende
				(eine <code>&lt;script&gt;</code>-Zeile und zehn <code>&lt;exports&gt;</code>-Zeilen).<br><br>

				<b>Wichtig:</b> Nach einer Änderung an der <code>meta.xml</code> reicht ein Neuladen nicht —
				in der Server-Konsole <code>restart '.htmlspecialchars(MTA_RESOURCE_NAME).'</code> eingeben
				oder den Server neu starten.<br><br>

				<small>Direkt testen kannst Du es im Browser auf dem Server:<br>
				<code>http://127.0.0.1:'.htmlspecialchars(MTA_HTTP_PORT).'/'.htmlspecialchars(MTA_RESOURCE_NAME).'/call/listAllPlayers</code><br>
				Es fragt nach Benutzer und Passwort (die aus der cfg.php). Kommt danach wieder
				„Not found", fehlen die Exports. Kommt eine Liste oder <code>[]</code>, ist alles richtig.</small>');
		}

		return array("ok" => false, "wert" => "", "fehler" => "MTA-Server meldet: ".htmlspecialchars($meldung));
	}

	if (!is_array($antwort) || !isset($antwort[0])) {
		return array("ok" => false, "wert" => "", "fehler" => "Keine Antwort vom MTA-Server. Ist die Funktion \"".htmlspecialchars($funktion)."\" in der meta.xml mit http=\"true\" exportiert?");
	}

	return array("ok" => true, "wert" => (string)$antwort[0], "fehler" => "");
}

/*
	Zerlegt einen Waffen-Slot im ICE-Format "WaffenID|Munition"
	(userdata.Gunbox1-3 und inventar.Waffenslot1-3).

	Wichtig: Der Standardwert ist "0|0" - das ist ein LEERER Slot.
	Vorher wurde daraus Waffe 0 = "Faust" mit 0 Schuss angezeigt.
*/
function cpParseWeaponSlot($slot) {
	global $weaponNames;

	$leer = array("id" => "0", "name" => "Leer", "ammo" => "0", "empty" => true);
	if ($slot === null || $slot === "" || $slot === "0") { return $leer; }

	$teile = explode('|', (string)$slot);
	$id    = ((isset($teile[0])) ? (int)$teile[0] : 0);
	$ammo  = ((isset($teile[1])) ? (int)$teile[1] : 0);

	if ($id <= 0) { return $leer; }

	return array(
		"id"	=> $id,
		"name"	=> ((isset($weaponNames[$id])) ? $weaponNames[$id] : 'Waffe #'.$id),
		"ammo"	=> number_format($ammo, 0, "", "."),
		"empty"	=> false
	);
}
...
/*
	...
	Das Problem: Bist Du im Panel angemeldet und oeffnest eine fremde
	Webseite, kann die im Hintergrund ein Formular an Dein Panel schicken -
	Dein Browser sendet den Anmelde-Cookie automatisch mit. Ein Angreifer
	koennte so z.B. als Admin alle Bans loeschen oder Dein Passwort aendern,
	ohne Deine Zugangsdaten zu kennen.

	Die Loesung: Jedes Formular bekommt ein unsichtbares Feld mit einem
	Schluessel, den nur wer den Anmelde-Cookie kennt berechnen kann.
	Fehlt der Schluessel oder ist er falsch, wird die Aktion abgelehnt.
*/

// Der Schluessel fuer den aktuell angemeldeten Besucher.
function cpCsrfToken() {
	$cookie = ((isset($_COOKIE['cpauth'])) ? $_COOKIE['cpauth'] : 'kein-cookie');
	return hash_hmac('sha256', 'cp2-formular-schutz', $cookie);
}

// Das unsichtbare Feld fuer ein Formular. Direkt nach <form ...> einfuegen.
function cpCsrfFeld() {
	return '<input type="hidden" name="cptoken" value="'.cpCsrfToken().'" />';
}

/*
	Prueft den Schluessel einer abgeschickten Aktion.
	Nimmt POST und GET (fuer die AJAX-Aufrufe).

	hash_equals vergleicht zeitkonstant - so laesst sich der Schluessel
	nicht durch Messen der Antwortzeit erraten.
*/
function cpCsrfOk() {
	$gesendet = '';
	if (isset($_POST['cptoken'])) { $gesendet = $_POST['cptoken']; }
	else if (isset($_GET['cptoken'])) { $gesendet = $_GET['cptoken']; }

	if (!is_string($gesendet) || $gesendet === '') { return false; }
	return hash_equals(cpCsrfToken(), $gesendet);
}

/*
	Bricht ab, wenn der Schluessel fehlt. Fuer Seiten, die etwas aendern.
	Rueckgabe: true = alles in Ordnung, false = Meldung wurde ausgegeben.
*/
function cpCsrfPruefen() {
	if (cpCsrfOk()) { return true; }

	echo '<div class="alert alert-danger">
		<b>Die Aktion wurde abgebrochen.</b><br>
		Der Sicherheitsschlüssel des Formulars war falsch oder fehlte.
		Das passiert, wenn die Seite sehr lange offen war oder Du zwischendurch
		neu angemeldet wurdest — lade die Seite neu und versuche es nochmal.
	</div>';
	return false;
}
...
/*
	=====================================================================
	Bremse gegen Passwort-Raten am Login
	=====================================================================

	Ohne Bremse kann ein Angreifer beliebig viele Passwoerter pro Sekunde
	durchprobieren. Hier werden Fehlversuche pro IP-Adresse gezaehlt und
	nach zu vielen Versuchen wird der Login kurz gesperrt.

	Gespeichert wird in logs/loginversuche.json - keine Datenbank noetig.
*/
define('CP_LOGIN_MAX_VERSUCHE', 5);		// so viele Fehlversuche sind erlaubt
define('CP_LOGIN_SPERRE', 600);			// danach so viele Sekunden Pause (600 = 10 Minuten)

function cpLoginDateiPfad() {
	$dir = dirname(__FILE__).DIRECTORY_SEPARATOR.'logs';
	if (!is_dir($dir)) { @mkdir($dir, 0777, true); }
	return $dir.DIRECTORY_SEPARATOR.'loginversuche.json';
}

function cpLoginVersucheLaden() {
	$datei = cpLoginDateiPfad();
	if (!is_file($datei)) { return array(); }

	$inhalt = @file_get_contents($datei);
	if ($inhalt === false || $inhalt === '') { return array(); }

	$daten = @json_decode($inhalt, true);
	if (!is_array($daten)) { return array(); }

	// Alte Einträge wegwerfen
	$jetzt = time();
	foreach ($daten as $ip => $eintrag) {
		if (!isset($eintrag['zeit']) || ($jetzt - $eintrag['zeit']) > CP_LOGIN_SPERRE) {
			unset($daten[$ip]);
		}
	}
	return $daten;
...
function cpLoginVersucheSpeichern($daten) {
	@file_put_contents(cpLoginDateiPfad(), json_encode($daten));
}

/*
	Wie lange ist diese IP noch gesperrt? 0 = nicht gesperrt.
*/
function cpLoginGesperrt() {
	$ip = ((isset($_SERVER['REMOTE_ADDR'])) ? $_SERVER['REMOTE_ADDR'] : '0.0.0.0');
	$daten = cpLoginVersucheLaden();

	if (!isset($daten[$ip])) { return 0; }
	if ((int)$daten[$ip]['anzahl'] < CP_LOGIN_MAX_VERSUCHE) { return 0; }

	$rest = CP_LOGIN_SPERRE - (time() - (int)$daten[$ip]['zeit']);
	return (($rest > 0) ? $rest : 0);
}

// Wie viele Fehlversuche hat diese IP schon?
function cpLoginVersucheAnzahl() {
	$ip = ((isset($_SERVER['REMOTE_ADDR'])) ? $_SERVER['REMOTE_ADDR'] : '0.0.0.0');
	$daten = cpLoginVersucheLaden();
	return ((isset($daten[$ip]['anzahl'])) ? (int)$daten[$ip]['anzahl'] : 0);
}

// Nach einem falschen Passwort aufrufen.
function cpLoginFehlversuch() {
	$ip = ((isset($_SERVER['REMOTE_ADDR'])) ? $_SERVER['REMOTE_ADDR'] : '0.0.0.0');
	$daten = cpLoginVersucheLaden();

	if (!isset($daten[$ip])) {
		$daten[$ip] = array("anzahl" => 0, "zeit" => time());
	}
	$daten[$ip]['anzahl'] = (int)$daten[$ip]['anzahl'] + 1;
	$daten[$ip]['zeit'] = time();

	cpLoginVersucheSpeichern($daten);
	cpWriteLog("login.log", "Fehlgeschlagener Login von ".$ip." (Versuch ".$daten[$ip]['anzahl'].")");
}

// Nach einem erfolgreichen Login aufrufen.
function cpLoginErfolg() {
	$ip = ((isset($_SERVER['REMOTE_ADDR'])) ? $_SERVER['REMOTE_ADDR'] : '0.0.0.0');
	$daten = cpLoginVersucheLaden();
	if (isset($daten[$ip])) {
		unset($daten[$ip]);
...
/*
	Setzt einen Cookie mit Schutzeinstellungen:

	httponly = JavaScript kann den Cookie nicht lesen (schuetzt bei XSS)
	samesite = der Cookie wird bei Aufrufen von fremden Seiten nicht
	           mitgeschickt (schuetzt gegen CSRF)
	secure   = nur ueber HTTPS senden (automatisch, wenn die Seite per
	           HTTPS laeuft)
*/
// Der Ordner, in dem das Panel liegt - z.B. "/CP2-Reallife/"
function cpCookiePfad() {
	$pfad = str_replace('\\', '/', dirname((isset($_SERVER['SCRIPT_NAME'])) ? $_SERVER['SCRIPT_NAME'] : '/'));
	return rtrim($pfad, '/').'/';
}

/*
	$dauer === null  => Session-Cookie: kein festes Ablaufdatum, verschwindet
	                     automatisch, sobald der Browser geschlossen wird.
	                     (Login soll IMMER neu noetig sein.)
	$dauer > 0       => Cookie haelt so viele Sekunden.
	$dauer <= 0       => Cookie sofort loeschen (Logout).
*/
function cpSetzeCookie($name, $wert, $dauer) {
	...
	// Ab PHP 7.3 kann man die Optionen als Array uebergeben.
	if (PHP_VERSION_ID >= 70300) {
		return setcookie($name, $wert, array(
			"expires"	=> $expires,
			"path"		=> $pfad,
			"secure"	=> $https,
			"httponly"	=> true,
	...
	// Aeltere PHP-Versionen: samesite ueber den Pfad anhaengen.
	return setcookie($name, $wert, $expires, $pfad."; samesite=Lax", "", $https, true);
}

/*
	Raeumt Cookies auf, die eine frühere Version dieses Panels mit dem Pfad
	"/" gesetzt hat.

	Warum das noetig ist: Es koennen dann ZWEI Cookies mit demselben Namen
	im Browser liegen (einer fuer "/CP2-Reallife/", einer fuer "/"). Welcher
	davon bei PHP ankommt, ist nicht garantiert - dadurch konnte die
	Anmeldung scheinbar grundlos fehlschlagen.

	Wird beim Login und beim Logout aufgerufen.
*/
function cpAlteCookiesAufraeumen() {
	$namen = array("cpauth", "cpuser", "lastInfo");
	foreach ($namen as $name) {
		// Nur loeschen, wenn der Pfad ueberhaupt ein anderer ist.
		if (cpCookiePfad() != "/") {
...
/*
	Findet den Ordner mit den ICE-Logdateien.

	ICE schreibt seine Logs als Dateien nach
	<Resource>/vio_stored_files/logs/<name>.log  (admin/allround_log.lua).
	Liegt das Panel auf demselben Rechner wie der MTA-Server, kann es die
	Dateien direkt lesen - dafuer braucht es keine MTA-Verbindung.

	Reihenfolge:
	1. der Pfad aus cfg.php (ICE_RESOURCE_PFAD)
	2. die ueblichen Stellen (MTA-Standardinstallation, Nachbarordner)

	Rueckgabe: Pfad zum logs-Ordner oder false
*/
function cpIceLogPfad() {
	static $gefunden = null;
	if ($gefunden !== null) { return $gefunden; }

	$kandidaten = array();

	// 1. Eigene Angabe aus cfg.php
	if (defined('ICE_RESOURCE_PFAD') && ICE_RESOURCE_PFAD != '') {
		$kandidaten[] = rtrim(ICE_RESOURCE_PFAD, '\\/');
	}

	// 2. Uebliche Stellen
	$panelOrdner = dirname(__FILE__);
	$resName = ((defined('MTA_RESOURCE_NAME')) ? MTA_RESOURCE_NAME : 'ICE');

	$kandidaten[] = dirname($panelOrdner).DIRECTORY_SEPARATOR.'ICE-Reallife';
	$kandidaten[] = dirname($panelOrdner).DIRECTORY_SEPARATOR.$resName;
	$kandidaten[] = 'C:\\MTA San Andreas 1.6\\server\\mods\\deathmatch\\resources\\'.$resName;
	$kandidaten[] = 'C:\\Program Files (x86)\\MTA San Andreas 1.6\\server\\mods\\deathmatch\\resources\\'.$resName;
	$kandidaten[] = 'C:\\Program Files\\MTA San Andreas 1.6\\server\\mods\\deathmatch\\resources\\'.$resName;
	$kandidaten[] = 'C:\\Users\\emres\\Desktop\\ICE-Reallife';

	foreach ($kandidaten as $pfad) {
		$logs = $pfad.DIRECTORY_SEPARATOR.'vio_stored_files'.DIRECTORY_SEPARATOR.'logs';
		if (is_dir($logs)) {
			$gefunden = $logs;
			return $gefunden;
		}
...
/*
	Liest eine ICE-Logdatei direkt von der Festplatte.
	Neueste Zeilen zuerst, maximal die letzten ~200 KB.

	Rueckgabe: Text oder false
*/
function cpLeseLogDatei($logname) {
	$ordner = cpIceLogPfad();
	if ($ordner === false) { return false; }

	// Nur Buchstaben, Zahlen, - und _ zulassen (kein ../ o.ae.)
	if (!preg_match('/^[A-Za-z0-9_-]+$/', $logname)) { return false; }

	$datei = $ordner.DIRECTORY_SEPARATOR.$logname.'.log';
	...
	if ($groesse > $maximal) { fseek($fp, $groesse - $maximal); }
	$inhalt = stream_get_contents($fp);
	fclose($fp);

	if ($inhalt === false || $inhalt === '') { return ''; }

	// Neueste Zeilen nach oben
	$zeilen = preg_split("/\r\n|\n|\r/", trim($inhalt));
...
/*
	=====================================================================
	Wer ist gerade im Spiel? - direkt beim MTA-Server nachfragen
	=====================================================================

	Jeder MTA-Server antwortet auf dem "ASE"-Port (Spielport + 123) mit
	seinem Namen, der Spielerzahl UND der Spielerliste. Genau darüber
	finden auch die Server-Browser die Spieler.

	Das Schoene daran:
	  * kein Passwort, keine ACL, keine mtaserver.conf-Aenderung noetig
	  * unabhaengig von der Datenbank und von der Tabelle "loggedin"
	  * funktioniert sofort, ohne den Server neu zu starten
	...
	Rueckgabe:
	  array(
	    ...
	    "anzahl"  => 3,
	    "max"     => 100,
	    "spieler" => array("PlayasEmre", "Testuser", ...)
	  )
	oder false, wenn der Server nicht antwortet.
*/
function cpMtaAseAbfrage() {
	static $ergebnis = null;
	if ($ergebnis !== null) { return $ergebnis; }

	/*
		Zwischenspeicher, damit nicht jeder Seitenaufruf beim Server anfragt:
		 * Erfolg  ->  5 Sekunden (Liste bleibt aktuell)
		 * Fehler  -> 30 Sekunden

		Der Fehlerfall MUSS auch gespeichert werden. Sonst wuerde bei einem
		abgeschalteten Spielserver jeder Seitenaufruf eine Sekunde auf die
		Zeitueberschreitung warten - das Panel waere spuerbar langsam.
	*/
	$cacheDatei = dirname(__FILE__).DIRECTORY_SEPARATOR.'logs'.DIRECTORY_SEPARATOR.'serverstatus.json';

	if (is_file($cacheDatei)) {
		$alter = time() - filemtime($cacheDatei);
		$daten = @json_decode((string)@file_get_contents($cacheDatei), true);

		if (is_array($daten)) {
			$maxAlter = ((!empty($daten['ok'])) ? 5 : 30);
			if ($alter < $maxAlter) { $ergebnis = $daten; return $ergebnis; }
		}
	}

	// Kleine Hilfe, damit jedes Ergebnis auch im Zwischenspeicher landet.
	$merken = function($wert) use ($cacheDatei) {
		@file_put_contents($cacheDatei, json_encode($wert));
		return $wert;
	};

	$port = (int)MTA_PORT + 123;
	$fp = @fsockopen('udp://'.MTA_IP, $port, $errno, $errstr, 1);

	if (!$fp) {
		$ergebnis = $merken(array("ok" => false, "fehler" => 'Keine Verbindung zu '.MTA_IP.':'.$port.' ('.$errstr.')'));
		return $ergebnis;
	}

	@stream_set_timeout($fp, 1);
	@fwrite($fp, 's');
	$rohdaten = @fread($fp, 32768);
	@fclose($fp);

	if (!$rohdaten || strlen($rohdaten) < 10 || substr($rohdaten, 0, 4) != 'EYE1') {
		$ergebnis = $merken(array("ok" => false, "fehler" => 'Der Server hat nicht geantwortet (UDP-Port '.$port.' erreichbar?)'));
		return $ergebnis;
	}

	/*
		Aufbau der Antwort: hinter "EYE1" folgen Textstuecke.
		Vor jedem Text steht ein Byte mit der Laenge + 1.
	*/
	$rest = substr($rohdaten, 4);

	$leseText = function() use (&$rest) {
		if (strlen($rest) < 1) { return false; }
		$laenge = ord($rest[0]) - 1;
		if ($laenge < 0 || strlen($rest) < 1 + $laenge) { return false; }
		$text = substr($rest, 1, $laenge);
		$rest = substr($rest, 1 + $laenge);
		return $text;
	};

	// Reihenfolge laut MTA: Spiel, Port, Servername, Spieltyp, Karte, Version, Passwort, Spieler, Maximum
	$felder = array();
	for ($i = 0; $i < 9; $i++) {
		$wert = $leseText();
		if ($wert === false) {
			$ergebnis = $merken(array("ok" => false, "fehler" => 'Antwort des Servers unvollstaendig.'));
			return $ergebnis;
		}
		$felder[] = $wert;
	}

	// Danach die Server-Einstellungen, beendet von einem Byte mit dem Wert 1
	$schutz = 0;
	while (strlen($rest) > 0 && ord($rest[0]) != 1 && $schutz < 200) {
		$schutz++;
		if ($leseText() === false) { break; }	// Schluessel
		if ($leseText() === false) { break; }	// Wert
	}
	if (strlen($rest) > 0 && ord($rest[0]) == 1) { $rest = substr($rest, 1); }

	// Und zum Schluss die Spieler
	$spieler = array();
	$schutz = 0;
	while (strlen($rest) > 1 && $schutz < 500) {
		$schutz++;
		...
		$name = '';
		foreach (array(1, 2, 4, 8, 16, 32) as $bit) {
			if (($flags & $bit) == 0) { continue; }
			$wert = $leseText();
			if ($wert === false) { break 2; }
			if ($bit == 1) { $name = $wert; }
		}

		if ($name !== '') {
			// MTA-Farbcodes wie #FF0000 aus dem Namen entfernen
			$spieler[] = trim(preg_replace('/#[0-9A-Fa-f]{6}/', '', $name));
		}
	}

	$ergebnis = $merken(array(
		"ok"		=> true,
		"name"		=> $felder[2],
		"anzahl"	=> (int)$felder[7],
		"max"		=> (int)$felder[8],
...
/*
	Ist dieser Spieler laut Spielserver gerade online?
	Vergleicht ohne Gross-/Kleinschreibung.
*/
function cpIstImSpiel($name) {
	$status = cpMtaAseAbfrage();
	if (!$status || empty($status['ok'])) { return null; }	// null = unbekannt

	foreach ($status['spieler'] as $sp) {
		if (strtolower($sp) == strtolower($name)) { return true; }
	}
	return false;
}

/*
	Ist dieser Spieler gerade ingame eingeloggt?

	Wichtig fuer alles, was das Panel in "userdata" schreibt (Coins, Premium,
	Skin, ...): ICE haelt die Werte waehrend des Spielens im Speicher und
	schreibt sie zurueck. Eine Aenderung ueber das Panel waere dann verloren.
	Deshalb gehen solche Aktionen nur bei Spielern, die offline sind.
*/
function cpSpielerIstOnline($con, $name) {

	// 1) Direkt beim Spielserver nachfragen (zuverlaessigste Quelle)
	$ase = cpIstImSpiel($name);
	if ($ase === true) { return true; }

	// 2) Tabelle loggedin
	$sql = $con->query("SELECT l.Loggedin AS Loggedin
		FROM players p JOIN loggedin l ON l.UID = p.UID
		WHERE p.Name='".$con->escape_string($name)."'");

	while ($sql && $row = $sql->fetch_assoc()) {
		if ($row['Loggedin'] == "1") { return true; }
	}
	return false;
}

/*
	Zaehlt Datensaetze und liefert 0, wenn die Abfrage fehlschlaegt.
	Verhindert "Call to a member function num_rows() on bool", falls eine
	Tabelle in der Datenbank fehlt.
*/
function cpCountRows($con, $sql) {
	$res = @$con->query($sql);
	if (!$res) { return 0; }
	return $res->num_rows;
}
...
/*
	Die ersten zwei Bloecke der IP-Adresse. Wird fuer den Auth-Cookie benutzt.

	Wichtig: Auf localhost liefert PHP oft "::1" (IPv6). Ein explode('.') haette
	dort keinen zweiten Block -> PHP-8-Warnung "Undefined array key 1".
	Deshalb wird IPv6 hier gesondert behandelt.
*/
function cpIpPrefix() {
	$ip = ((isset($_SERVER['REMOTE_ADDR'])) ? $_SERVER['REMOTE_ADDR'] : '0.0.0.0');

	if (strpos($ip, ':') !== false) {
		// IPv6: die ersten zwei Gruppen verwenden.
		$ex = explode(':', $ip);
		return $ex[0].":".((isset($ex[1])) ? $ex[1] : '');
	}
...
// Schreibt eine Zeile in eine Logdatei im Ordner logs/ (legt den Ordner bei Bedarf an).
function cpWriteLog($file, $line) {
	$dir = dirname(__FILE__).DIRECTORY_SEPARATOR.'logs';
	if (!is_dir($dir)) { @mkdir($dir, 0777, true); }

	// Zeilenumbrueche raus - sonst koennte z.B. ein Kick-Grund mit \n
	// gefaelschte Log-Zeilen einschleusen (Log-Injection).
	$line = str_replace(array("\r", "\n"), ' ', $line);

	@file_put_contents($dir.DIRECTORY_SEPARATOR.$file, "[".date("d-m-Y, H:i:s")."]: ".$line."\n", FILE_APPEND);
}
...
/*
	Rechnet einen ICE-Zeitwert (getSecTime) zurueck in ein Datum.

	ICE speichert z.B. players.LastLogin und ban.STime als
	"Jahre-seit-1900 * 365*24*60 + Tag-im-Jahr * 24*60 + Stunde * 60 + Minute".
	Praktisch fuer "Letzter Login", wenn players.Last_login (Text) leer ist.

	Hinweis: ICE rechnet mit 365-Tage-Jahren, Schalttage fehlen also -
	das Datum kann ein paar Tage abweichen. Uhrzeit und Jahr stimmen.
*/
function cpSecTimeZuDatum($wert) {
	$wert = (int)$wert;
	if ($wert <= 0) { return false; }

	$minutenProJahr = 365 * 24 * 60;

	$jahre	= (int)floor($wert / $minutenProJahr);
	$rest	= $wert % $minutenProJahr;
	$tag	= (int)floor($rest / (24 * 60));
	$rest2	= $rest % (24 * 60);
	$stunde	= (int)floor($rest2 / 60);
	$minute	= $rest2 % 60;

	$jahr = 1900 + $jahre;
	if ($jahr < 2000 || $jahr > 2100) { return false; } // unplausibel

	$zeit = mktime($stunde, $minute, 0, 1, 1 + $tag, $jahr);
	if ($zeit === false) { return false; }
