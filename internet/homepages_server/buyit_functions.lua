...
function buyItGetFreeCarslot ( pname )
	...
	for i = 1, 10 do
		if MtxGetElementData ( player, "carslot"..i ) == 0 then
			return i
		elseif count >= MtxGetElementData ( player, "maxcars" ) then
			return false
		else
			count = count + 1
		end
...
function buyItMoneyChange ( pname, amount )
	...
	if isElement ( player ) then
		local money = MtxGetElementData ( player, "bankmoney" )
		MtxSetElementData ( player, "bankmoney", money + amount )
	elseif playerUID[pname] then
		local money = tonumber ( (dbQueryCoro ( "SELECT ?? FROM ?? WHERE ??=?", "Bankgeld", "players", "UID", playerUID[pname] ))[1]["Bankgeld"] )
