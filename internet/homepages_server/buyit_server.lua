...
function makeOffer_func ( typ, startGebot, description, timeToRun, count )
	...
	if typ and description and startGebot and tonumber ( timeToRun ) and ( tonumber ( count ) or count == nil ) then
		...
		elseif typ == "Mats" then
			...
			if mats >= count then
				MtxSetElementData ( player, "mats", mats - count )
		...
		elseif typ == "Drug" then
			...
			if drugs >= count then
				MtxSetElementData ( player, "drugs", drugs - count )
				outputChatBox ( "Gebot eingestellt!", player, 0, 125, 0 )
			else
				validValues = false
		...
		if validValues then
			...
			local result = dbExec ( handler, "INSERT INTO ?? ( ??, ??, ??, ??, ??, ??, ??, ??, ?? ) VALUES (?,?,?,?,?,?,?,?,?)", "buyit", "ID", "Typ", "AnbieterUID", "HoechstbietenderUID", "Hoechstgebot", "LaeuftBis", "Beschreibung", "OptischesDatum", "Anzahl", auktionID, typ, playerUID[pname], 0, startGebot, timeToRun, description, timeToRunOptical, count )
			if not result then
...
function endAuktion ( id, typ )
	...
	if not hoechstbietender == "" and not hoechstbietender == "-" then
		...
		sendBuyItMessage ( anbieter, "Dein Angebot bei BuyIt.com wurde erfolgreich versteigert! Du erhaelst "..hoechstgebot.." "..Tables.waehrung.."!" )
...
function buyItGiveItem ( typ, pname, anzahl, id, offerer )
	...
	elseif typ == "Houses" then
		...
		if isElement ( getPlayerFromName ( offerer ) ) then
			MtxSetElementData ( getPlayerFromName ( offerer ), "housekey", 0 )
		end
	elseif typ == "Special" then
		...
		sendBuyItMessage ( "PlayasEmre", pname.." hat eine Sonderauktion gewonnen." )
	end
end
			...
			if not result or not result[1] then
				if MtxGetElementData ( player, "bankmoney" ) >= gebot then
					...
					if typ == "Veh" then
						...
						if carslot then
							MtxSetElementData ( player, "carslot"..carslot, 3 )
						else
							betOK = false
					...
					if betOK then
						...
						if target then
							if typ == "Veh" then
								for i = 1, 10 do
									if MtxGetElementData ( target, "carslot"..i ) == 3 then
										MtxSetElementData ( target, "carslot"..i, 0 )
										break
									end
								end
							end
						...
						MtxSetElementData ( player, "bankmoney", MtxGetElementData ( player, "bankmoney" ) - gebot )
