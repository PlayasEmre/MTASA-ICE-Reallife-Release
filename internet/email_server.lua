...
function sendMail_func ( text, betreff, to )
	...
	if exists then
		local mail = tostring ( getPlayerName(client).."@Infinite-RL.de|"..betreff.."|"..timestamp()..": "..text )
		...
		if isElement ( getPlayerFromName ( to ) ) and MtxGetElementData ( getPlayerFromName ( to ), "loggedin" ) == 1 then
			getMailsForClient_func ( to )
		end
	else
		outputChatBox ( "Ungueltige Eingabe, bitte verwende folgende Eingabe: [Name@Infinite-RL.de]", client, 125, 0, 0 )
	end
end
