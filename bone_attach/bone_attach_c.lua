...
function putAttachedElementsOnBones()
	-- Die Offset-Tabellen sind globale Variablen ( attach_func.lua ). Sie wurden
	-- bisher fuer JEDES angehaengte Element in JEDEM Frame acht Mal einzeln aus
	-- der globalen Umgebung geholt. Die Tabellen selbst werden nie ausgetauscht,
	-- nur ihr Inhalt - deshalb reicht es, die Verweise einmal pro Frame zu holen.
	local aPed, aBone = attached_ped, attached_bone
	local aX, aY, aZ = attached_x, attached_y, attached_z
	local aRX, aRY, aRZ = attached_rx, attached_ry, attached_rz
	local basisKnochen = bone_0

	for element,ped in pairs(aPed) do
		if not isElement(element) then
			clearAttachmentData(element)
		elseif isElementStreamedIn(ped) then
			local bone = aBone[element]
			local x,y,z = getPedBonePosition(ped,basisKnochen[bone])
			local xx,xy,xz,yx,yy,yz,zx,zy,zz = getBoneMatrix(ped,bone)
			local offx,offy,offz = aX[element],aY[element],aZ[element]
			local offrx,offry,offrz = aRX[element],aRY[element],aRZ[element]
			...
			if (objx ~= objx or objy ~= objy or objz ~= objz) then
			  setElementPosition(element, x, y, z)
			else
			  setElementPosition(element, objx, objy, objz)
			end
			if (not (offrx ~= offrx or offry ~= offry or offrz ~= offrz)) then
