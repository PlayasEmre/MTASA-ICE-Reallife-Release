...
function damagePlayer ( player, amount, damager, weapon )
	...
	if isElement ( player ) then
		...
		if not died then
			MtxSetElementData ( player, "lasthp", getElementHealth ( player )+ getPedArmor ( player ))
		end
		MtxSetElementData ( player, "DamageBekommen", MtxGetElementData ( player, "DamageBekommen" ) + amount )
		MtxSetElementData ( damager, "DamageGemacht", MtxGetElementData ( damager, "DamageGemacht" ) + amount )
	end
end
...
function formNumberToMoneyString ( value )
	...
	if tonumber ( value ) then
		...
		if string.sub ( value, 1, 1 ) == "-" then
			return "-"..setDotsInNumber ( string.sub ( value, 2, #value ) )..""..Tables.waehrung..""
		else
			return setDotsInNumber ( value )..""..Tables.waehrung..""
		end
	end
	return false
end
...
function toggleVehicleTrunk ( veh )
	...
	if MtxGetElementData ( veh, "stuning1" ) then
		...
		local state = MtxGetElementData ( veh, "trunkState" )
		if MtxGetElementData ( veh, "trunkState" ) == nil then
			MtxSetElementData ( veh, "trunkState", false )
			setVehicleDoorState ( veh, 1, 0 )
			state = false
		end
		MtxSetElementData ( veh, "trunkState", not state )
		if state then
			setVehicleDoorState ( veh, 1, 0 )
		else
...
function getVehicleTrunkState ( veh )
	...
	if MtxGetElementData ( veh, "trunkState" ) then
		return true
	end
	return false
end
...
function fadeElementInterior ( player, int, intx, inty, intz, rot, dim )
	if player and isElement ( player ) and getElementType ( player ) == "player" then
	   triggerClientEvent(player,"blendLoadingScreen",player)
		if not MtxGetElementData ( player, "intchange" ) then
			MtxSetElementData ( player, "intchange", true )
			if lp then
				...
				setTimer ( MtxSetElementData, 2000, 1, player, "intchange", false )
			else
