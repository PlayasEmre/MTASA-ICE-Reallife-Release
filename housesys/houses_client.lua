	...
	if isElement ( pickup ) then
		...
		if getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 ) <= houseInfoTextRange then
			...
			if not getElementData ( pickup, "gangHQOf" ) then
				...
				local tier = getElementData ( pickup, "tier" )
				local tierText = ""
				if tier == "billig" then
					tierText = "Billige Wohnung"
				elseif tier == "teuer" then
					tierText = "Teure Wohnung"
				elseif tier == "mittel" then
					tierText = "Mittlere Wohnung"
				end
