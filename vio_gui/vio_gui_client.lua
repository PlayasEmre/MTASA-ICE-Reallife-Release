...
-- Ressource (siehe resources/DGS) als globale Funktionen. Muss als
-- allererstes passieren, bevor irgendein Skript dgsCreate* aufruft - diese
-- Datei laedt laut meta.xml als erste client-Datei nach den MySQL-Skripten.
-- Voraussetzung auf dem Server: die Ressource "DGS" muss gestartet sein
-- (siehe DGS/meta.xml <aclrequest> fuer die dafuer noetigen ACL-Rechte).
...
if not dgsOk then
	...
	outputChatBox("#FF5555[ICE] Fehler: DGS-Ressource nicht gefunden oder nicht gestartet. GUI wird nicht funktionieren!", localPlayer, 255, 255, 255, true)
else
	-- Eigenes Farbschema aktivieren (siehe dgsstyle/styleSettings.txt) -
	-- ueberschreibt nur die dort gesetzten Werte, Rest bleibt DGS-Standard.
	local styleOk, styleErr = pcall(function()
		dgsAddStyle("ICE", "dgsstyle")
		dgsSetStyle("ICE")
	end)
	if not styleOk then
		outputDebugString("[ICE] Konnte eigenes DGS-Farbschema nicht laden: "..tostring(styleErr), 1)
		outputChatBox("#FF5555[ICE] Fehler beim Laden des DGS-Farbschemas: "..tostring(styleErr), localPlayer, 255, 255, 255, true)
	end
end
...
function vioGuiCreateProgressBar ( x, y, width, height, bool, startValue, parent )
	...
	local base = guiCreateStaticImage ( x, y, width, height, ":"..getResourceName(getThisResource()).."/images/gui/bar/bar_c1.png", false, parent )
	guiCreateStaticImage ( 0, 0, width, pxSizeH, ":"..getResourceName(getThisResource()).."/images/colors/c_black.jpg", false, base ) -- upper bar
	guiCreateStaticImage ( 0, height - pxSizeH, width, pxSizeH, ":"..getResourceName(getThisResource()).."/images/colors/c_black.jpg", false, base ) -- bottom bar
	guiCreateStaticImage ( 0, 0, pxSizeW, height, ":"..getResourceName(getThisResource()).."/images/colors/c_black.jpg", false, base ) -- left bar
	guiCreateStaticImage ( width - pxSizeW, 1, pxSizeW, height, ":"..getResourceName(getThisResource()).."/images/colors/c_black.jpg", false, base ) -- right bar
	
	progressBarData[base] = guiCreateStaticImage ( pxSizeW, pxSizeH * 2, ( width - 2 * ( pxSizeW ) ) * startValue, height - 2 * ( pxSizeH ) - 1, ":"..getResourceName(getThisResource()).."/images/gui/bar/bar_c2.png", false, base ) -- progress from 0 to 1
...
function guiDebugClick ()
	if getElementType ( source ) == "gui-staticimage" and not internetImages[source] and not ( colorSelectionImage == source ) then
