...
function Marker24_7Hit ( player, dim )
	if getElementType ( player ) == "player" and dim then
		if not getPedOccupiedVehicle ( player ) then
			MtxSetElementData(player, "24/7", "SF")
...
function Marker24_7_LV_Hit ( player, dim )
	...
	if getElementType ( player ) == "player" and dim then
		if not getPedOccupiedVehicle ( player ) then
			MtxSetElementData(player, "24/7", "LV")
			setElementDimension ( player, 1 )
			setElementInterior ( player, 6 )
			setCameraMatrix ( player, -24.504987716675, -50.165203094482, 1004.0047607422, -21.805931091309, -57.300312042236, 1005.2012329102 )
			triggerClientEvent ( player, "create24_7Shop", getRootElement() )
...
function Cancel24_7_func ( player )
	if player == client then
		if (MtxGetElementData(player,"24/7") == "LV") then
	...
	if player == client then
		...
		if item == "flowers" then
			if money >= flowers_price then
				MtxSetElementData ( player, "money", money - flowers_price )
		...
		elseif item == "cam" then
			if money >= cam_price then
				if cam then
					...
					MtxSetElementData ( player, "money", money - cam_price )
		...
		elseif item == "camammo" then
			if money >= camammo_price then
				if cam then
					MtxSetElementData ( player, "money", money - camammo_price )
					giveWeapon ( player, 43, 36, true )
					playSoundFrontEnd ( player, 40 )
					triggerClientEvent ( player, "infobox_start", getRootElement(), "\n\nGekauft!", 7500, 0, 125, 0 )
				else
		...
		elseif item == "nv" then
			if money >= nvgoogles_price then
				MtxSetElementData ( player, "money", money - nvgoogles_price )
		...
		elseif item == "t" then
			if money >= tgoogles_price then
				MtxSetElementData ( player, "money", money - tgoogles_price )
		...
		elseif item == "los" then
			if money >= rubbellos_price then
				MtxSetElementData ( player, "money", money - rubbellos_price )
				playSoundFrontEnd ( player, 40 )
				...
				elseif rnd <= 80 then
					...
					MtxSetElementData ( player, "money", MtxGetElementData(player,"money") + (rubbellos_price*1.5) )
				elseif rnd <= 95 then
					outputChatBox ( "Du hast "..(rubbellos_price*2).." $ gewonnen!", player, 125, 0, 0 )
					MtxSetElementData ( player, "money", MtxGetElementData(player,"money") + rubbellos_price*2 )
				elseif rnd <= 99 then
					outputChatBox ( "Du hast "..(rubbellos_price*5).." $ gewonnen!", player, 125, 0, 0 )
					MtxSetElementData ( player, "money", MtxGetElementData(player,"money") + rubbellos_price*5 )
				else
					outputChatBox ( "Du hast "..(rubbellos_price*20).." $ gewonnen!", player, 125, 0, 0 )
					MtxSetElementData ( player, "money", MtxGetElementData(player,"money") + rubbellos_price*20 )
				end
				triggerClientEvent ( player, "infobox_start", getRootElement(), "\n\nGekauft!", 7500, 0, 125, 0 )
			else
				triggerClientEvent ( player, "infobox_start", getRootElement(), "\n\nDu hast nicht\ngenug Geld!", 7500, 125, 0, 0 )
		...
		elseif item == "beer" then
			if money >= beer_price then
				MtxSetElementData ( player, "money", money - beer_price )
				playSoundFrontEnd ( player, 40 )
		...
		elseif item == "cig" then
			if money >= zigarett_price then
				MtxSetElementData ( player, "money", money - zigarett_price )
				MtxSetElementData ( player, "zigaretten", MtxGetElementData ( player, "zigaretten" ) + 5 )
				playSoundFrontEnd ( player, 40 )
			else
		...
		elseif item == "sim-50" or item == "sim-100" or item == "sim-250" then
			if MtxGetElementData ( player, "handyType" ) == 2 then
				...
				if money >= val then
					MtxSetElementData ( player, "handyCosts", MtxGetElementData ( player, "handyCosts" ) + val )
					infobox ( player, "Guthaben aufgeladen!\nTippe /call *100#\num dein Guthaben zu\nueberpruefen!", 5000, 125, 0, 0 )
					MtxSetElementData ( player, "money", MtxGetElementData ( player, "money" ) - val )
				else
...
function changeTarif ( val )
	...
	if val == 1 or val == 2 or val == 3 then
		if val ~= MtxGetElementData ( player, "handyType" ) then
			if MtxGetElementData ( player, "handyType" ) == 1 then
				if MtxGetElementData ( player, "money" ) >= MtxGetElementData ( player, "handyCosts" ) then
					MtxSetElementData ( player, "money", MtxGetElementData ( player, "money" ) - MtxGetElementData ( player, "handyCosts" ) )
			...
			if val == 1 then
				if MtxGetElementData ( player, "money" ) >= 10 then
					MtxSetElementData ( player, "money", MtxGetElementData ( player, "money" ) - 10 )
			...
			elseif val == 2 then
				MtxSetElementData ( player, "handyType", 2 )
				MtxSetElementData ( player, "handyCosts", 0 )
				infobox ( player, "Tarif gewechselt!\nTippe /call *100#\nfuer mehr Infos!", 5000, 0, 200, 0 )
			elseif val == 3 then
				MtxSetElementData ( player, "handyType", 3 )
				MtxSetElementData ( player, "handyCosts", 0 )
				infobox ( player, "Tarif gewechselt!\nTippe /call *100#\nfuer mehr Infos!", 5000, 0, 200, 0 )
			end
		else
