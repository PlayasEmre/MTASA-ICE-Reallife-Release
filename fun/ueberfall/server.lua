...
local function ueberfallTod ( ammo, killer )		
	if playerGettingRobbed[source] then
		if playerSchusserlaubnis[source] == nil and playerHatGeschossen[source] == nil then
			local themoney = 0
			outputChatBox ( "Zu früh angegriffen! Dir werden 1000"..Tables.waehrung.." abgezogen!", killer, 255, 0, 0 )
			if MtxGetElementData ( killer, "money" ) >= 1000 then 
				themoney = 1000
				MtxSetElementData ( killer, "money", MtxGetElementData ( killer, "money")+500 )
			elseif MtxGetElementData ( killer, "bankmoney" ) >= 1000 then 
				themoney = 1000
				MtxSetElementData ( killer, "bankmoney", MtxGetElementData ( killer, "bankmoney")+500 )
			elseif MtxGetElementData ( killer, "money" ) + MtxGetElementData ( killer, "bankmoney" ) >= 500 then
				local restgeld = 1000 - MtxGetElementData ( killer, "money" )
				themoney = 1000
				MtxSetElementData ( killer, "money", 0 )
				MtxSetElementData ( killer, "bankmoney", restgeld )
			else 
				themoney = MtxGetElementData ( killer, "bankmoney" ) + MtxGetElementData ( killer, "money" )
				MtxSetElementData ( killer, "money", 0 )
				MtxSetElementData ( killer, "bankmoney", 0)
			end
			MtxSetElementData ( source, "money", MtxGetElementData (source, "money")+themoney)
		end
...
local function startUeberfallZwei ( player, target )
	...
	for i=1, #players do
		...
		elseif playeritem == target then
			outputChatBox ( "Du wirst von der "..fraktionNames[frac].." überfallen (max. 500"..Tables.waehrung..")!", playeritem, 255, 0, 0 )
...
local function startUeberfallEins ( player, cmd, target )
	if isEvil ( player ) then
		if not MtxGetElementData ( player, "jailtime" ) or MtxGetElementData ( player, "jailtime" ) == 0 then
			if target then
				if findPlayerByName ( target )  then
					local targetpl = findPlayerByName ( target )
					local frac = MtxGetElementData ( player, "fraktion" )
					if MtxGetElementData ( targetpl, "fraktion" ) == 0 then
						if MtxGetElementData ( targetpl, "playingtime") >= 180 then
...
local function payTheRob ( player )
	if playerGettingRobbed[player] then
		...
		local themoney = 500
		if MtxGetElementData(player, "money") < 500 then
			themoney = MtxGetElementData(player, "money")
		end
		MtxSetElementData ( player, "money", MtxGetElementData (player, "money") - themoney )
		MtxSetElementData ( raeuber, "money", MtxGetElementData (raeuber, "money") + themoney )
		playerSchusserlaubnis[player] = nil
		playerHatGeschossen[player] = nil
		if isTimer ( playerGettingRobbedTimer[player] ) then
			killTimer ( playerGettingRobbedTimer[player] )
