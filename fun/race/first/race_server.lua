...
function raceFinished_func ( player, ms, s, m )
	if player == client then
		...
		if m < 2 or ( m == 2 and s < 10 ) then
			dbExec ( handler, "INSERT INTO ?? ( ??, ??, ??, ??, ??, ?? ) VALUES (?,?,?,?,?,?)", "ban", "UID", "AdminUID", "Grund", "Datum", "IP", "Serial", playerUID[getPlayerName ( player )], 0, 'Speedhack', timestamp(), getPlayerIP ( player ), getPlayerSerial ( player ) )
