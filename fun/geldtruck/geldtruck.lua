...
local function onGeldtruckExplode ( )
	...
	if isTimer ( geldtruckTimer ) then killTimer ( geldtruckTimer ) end
				...
				if veh == gTruck then  
					...
					for i=1, #players do
						if isOnDuty ( players[i] ) then
							MtxSetElementData ( players[i], "money", MtxGetElementData ( players[i], "money" ) + 5000 )
						end
					end
					setTimer(gOnDriveRestart,60*1000*60,1)
				end
...
local function gStart (player)
	if not Aktiondeaktivieren then
	   infobox ( player, "Gerade ist es nicht möglich einen geldtruck zu starten", 5000, 150, 0, 0 )
       return
	end
			...
			if gOnDrive == 0 then
				if not aktionlaeuft then
					...
					if hours >= 12 then
						...
						gOnDrive = 1 
						aktionlaeuft = true
						outputLog ( "Geldtruck - "..getPlayerName(player).." - "..fraktionNames[MtxGetElementData ( player, "fraktion")], "aktion" )
