...
function hitAHostage ( )
	if MtxGetElementData ( source, "money" ) >= 300 then
		outputChatBox ( "Verletze nicht die Zivilisten! Dafür werden dir 300$ abgezogen!", source, 255, 0, 0 )
		takePlayerSaveMoney ( source, 300 )
	elseif MtxGetElementData ( source, "bankmoney" ) >= 300 then
		outputChatBox ( "Verletze nicht die Zivilisten! Dafür werden dir 300$ von der Bank abgezogen!", source, 255, 0, 0 )
		MtxSetElementData ( source, "bankmoney", MtxGetElementData ( source, "bankmoney" ) - 300 )
	elseif MtxGetElementData ( source, "money" ) + MtxGetElementData ( source, "bankmoney" ) >= 300 then
		...
		MtxSetElementData ( source, "bankmoney", MtxGetElementData ( source, "bankmoney" ) - ( 300 - money ) )
	else
		local money = MtxGetElementData ( source, "money" ) + MtxGetElementData ( source, "bankmoney" )
		outputChatBox ( "Verletze nicht die Zivilisten! Dafür wird dir all dein restliches Geld weggenommen!", source, 255, 0, 0 )	
		takePlayerSaveMoney ( source, MtxGetElementData ( source, "money" ) )
		MtxSetElementData ( source, "bankmoney", 0 )
	end
end
...
function goInOutMuseum ( hitElement, dim )
	if getElementType ( hitElement ) == "player" and dim then
		...
		if not Aktiondeaktivieren then
		    infobox ( hitElement, "Gerade ist es nicht möglich einen Museumraub zu starten", 5000, 150, 0, 0 )
	        return
	    end
		...
		MtxSetElementData ( hitElement, "spawnProtection", true )
		setTimer ( MtxSetElementData, 1000, 1, hitElement, "spawnProtection", false )
	end
end
...
function startMuseumRob_click ( )
	if robberGangs[MtxGetElementData ( source, "fraktion" )] then
...
function startMuseumRob ( player )
	museumrobbing = true 
	robberfaction = MtxGetElementData ( player, "fraktion" )
	robberplayer = player
	addEventHandler ( "onPlayerWasted", player, museumRobPlayerWasted )
	addEventHandler ( "onPlayerQuit", player, museumRobPlayerWasted )
	outputLog ( "Museumraub - "..getPlayerName(player).." - "..fraktionNames[MtxGetElementData ( player, "fraktion")], "aktion" )
	animpedtyp = 1
	for thePlayer, _ in pairs ( playerInMuseum ) do
...
function museumVehicleEnter ( player, _, jacked, door )
	if MtxGetElementData ( player, "fraktion" ) ~= robberfaction then
