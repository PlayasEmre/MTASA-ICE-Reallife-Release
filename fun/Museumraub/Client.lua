...
function museumAlarmSound ( typ )
	...
	alarmsound = playSound ( ":"..getResourceName(getThisResource()).."/sounds/klingel.mp3", true )
...
function museumNormalSound ( )
	...
	createMuseumPeds()
	local path = ( museumsoundtype == 1 and ":"..getResourceName(getThisResource()).."/sounds/pianochill.mp3" ) or ( museumsoundtype == 2 and ":"..getResourceName(getThisResource()).."/sounds/fahrstuhlmusik.mp3" ) 
		...
		if getPedControlState ( source, "aim_weapon" ) then
			if element == museumpeds[1] then
				if not museumrobgoing and lasttargetedped + 2000 <= getTickCount() then
					...
					triggerServerEvent ( "startmuseumrob", localPlayer )
				end
			end
		end
	end
...
function tryToGetTheArtifact ( )
	...
	addEventHandler ( "onClientPlayerWasted", localPlayer, stopTryingToGetTheArtifact )
end
...
function stopTryingToGetTheArtifact ( )
	...
	removeEventHandler ( "onClientPlayerWasted", localPlayer, stopTryingToGetTheArtifact )
	artifactstatus = 0
end
...
function drawTheArtifactStatus ( )
	...
	if artifactstatus == 1000 then
		...
		triggerServerEvent ( "gottheartifact", localPlayer )
	else
