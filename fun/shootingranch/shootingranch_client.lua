...
-- Timer je Ziel. Vorher lag er unter _G[tostring(element).."Timer"] - beim
-- Aufraeumen ist das Element aber schon zerstoert und tostring() liefert dafuer
-- eine andere Zeichenkette, sodass killTimer nil bekam. Das Element selbst als
-- Tabellenschluessel bleibt dagegen auch nach dem Zerstoeren gueltig.
local zielTimer = {}
...
function moveTarget ( target, x2, y2 )
	...
	zielTimer[target] = setTimer (
		function ( element )
			if not isElement ( element ) then
				if isTimer ( zielTimer[element] ) then
					killTimer ( zielTimer[element] )
				end
				zielTimer[element] = nil
			elseif not flippingArray[element] then
				local x1, y1, z1 = elementPos[element]["x1"], elementPos[element]["y1"], elementPos[element]["z1"] 
				local x2, y2 = elementPos[element]["x2"], elementPos[element]["y2"]
				local z2 = z1
...
function showShootingRangeSelection_func ()
	...
	if gWindow["shootingRange"] then
		dgsSetVisible ( gWindow["shootingRange"], true )
		dgsBringToFront ( gWindow["shootingRange"] )
	else
		gWindow["shootingRange"] = dgsCreateWindow(screenwidth/2-251/2,screenheight/2-178,251,178,"Schiesstand",false)
		dgsBringToFront ( gWindow["shootingRange"] )
		dgsWindowSetCloseButtonEnabled ( gWindow["shootingRange"], false )
		dgsWindowSetSizable ( gWindow["shootingRange"], false )
		dgsWindowSetMovable ( gWindow["shootingRange"], false )

		gGrid["shootingRange"] = dgsCreateGridList(12,29,143,138,false,gWindow["shootingRange"])
		dgsGridListSetSelectionMode(gGrid["shootingRange"],0)

		gColumn["shootingRange"] = dgsGridListAddColumn(gGrid["shootingRange"],"Waffen",0.8)

		gButton["shootingRangeTest"] = dgsCreateButton(167,39,72,45,"Waffe testen",false,gWindow["shootingRange"], _, _, _, _, _, _, tocolor(46,110,255,255), tocolor(123,163,255,255), tocolor(24,74,199,255))
		dgsSetFont(gButton["shootingRangeTest"],"default-bold")
		gButton["shootingRangeClose"] = dgsCreateButton(167,107,72,45,"Schliessen",false,gWindow["shootingRange"], _, _, _, _, _, _, tocolor(46,110,255,255), tocolor(123,163,255,255), tocolor(24,74,199,255))
		dgsSetFont(gButton["shootingRangeClose"],"default-bold")

		addEventHandler ( "onDgsMouseClickUp", gButton["shootingRangeClose"],
			function (btn)
				if btn ~= "left" then return end
				dgsSetVisible ( gWindow["shootingRange"], false )
				guiSetInputMode ( "allow_binds" )
				showCursor ( false )
				setElementClicked ( false )
			end,
		false )
		addEventHandler ( "onDgsMouseClickUp", gButton["shootingRangeTest"],
			function (btn)
				if btn ~= "left" then return end
				local row = dgsGridListGetSelectedItem ( gGrid["shootingRange"] )
				if row == -1 then return end
				local gun = dgsGridListGetItemText ( gGrid["shootingRange"], row, gColumn["shootingRange"] )
		...
		for key, index in pairs ( shootingRangeGuns["name"] ) do
			...
			local row = dgsGridListAddRow ( gGrid["shootingRange"] )
			dgsGridListSetItemText ( gGrid["shootingRange"], row, gColumn["shootingRange"], name, false, false )
		end
	end
end
