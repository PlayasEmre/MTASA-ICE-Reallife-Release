...
addEventHandler("boxenstarten1", getRootElement(), function(player)
	...
	botErstellen( player )
	if not MtxGetElementData(player, "boxlvl") then
		MtxSetElementData(player, "boxlvl", 0)
	end
	if MtxGetElementData(player, "boxlvl") == 2 then
...
function boxerLebtNochFrage( player ) --Diese Funktion prüft nach ob der Boxer noch lebt/der spieler gewonnen hat
	if isPedDead(boxer[player]) then
		...
		if MtxGetElementData(player, "boxlvl") == 2 then
			...
			MtxSetElementData(player, "money", MtxGetElementData(player, "money") + 800)
			KampfstyledesBoxers[player] = nil
		elseif MtxGetElementData(player, "boxlvl") == 1 then
			MtxSetElementData(player, "boxlvl", 2)
			variable[player] = 155
		...
		elseif MtxGetElementData(player, "boxlvl") == 0 then
			MtxSetElementData(player, "boxlvl", 1)
			variable[player] = 230
