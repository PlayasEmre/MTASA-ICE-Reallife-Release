...
local function startrob ( player )
	...
	outputLog ( "Carrob - "..getPlayerName(player).." - "..fraktionNames[MtxGetElementData ( player, "fraktion")], "aktion" )
...
addCommandHandler("carrob", function(player, cmd)
	if not Aktiondeaktivieren then
	   infobox ( player, "Gerade ist es nicht möglich einen CarRob zu starten", 5000, 150, 0, 0 )
	   return
    end
