...
function presidentGotWasted ( _, killer )
	...
	if isElement ( killer ) and getElementType ( killer ) == "player" then
		local frac = MtxGetElementData ( killer, "fraktion" )
		...
		outputLog ( "Präsi-Konvoi - "..getPlayerName(killer).." hat Präsi getötet - "..fraktionNames[MtxGetElementData(killer,"fraktion")], "aktion" )
	end
	setTimer ( function ( )
...
function startPresiKonvoi ( player )
	if not Aktiondeaktivieren then
	   infobox ( player, "Gerade ist es nicht möglich einen President-Konvoi zu starten", 5000, 150, 0, 0 )
       return
	end
	...
	outputLog ( "Präsi-Konvoi - "..fraktionNames[MtxGetElementData(player,"fraktion")], "aktion" )
