...
function drogentruckEntrance_func ( player )
	...
	if not Aktiondeaktivieren then
	   infobox ( player, "Gerade ist es nicht möglich einen drogentruck zu starten", 5000, 150, 0, 0 )
       return
	end
...
function loadDrogenTruck ( player )
	...
	outputLog ( "Drogentruck - "..getPlayerName(player).." - "..fraktionNames[MtxGetElementData ( player, "fraktion")], "aktion" )
	addEventHandler ( "onVehicleExplode", drogentruck, vehicleDestroyedDrogenTruck )
	drogentruckFraktion = MtxGetElementData ( player, "fraktion" )
...
function drogentruckSuccessfull ( player )
	...
	MtxSetElementData ( player, "flowerseeds", MtxGetElementData (player, "flowerseeds") + 50 )
	MtxSetElementData ( player, "drugs", MtxGetElementData (player, "drugs") + 1500 )
end
