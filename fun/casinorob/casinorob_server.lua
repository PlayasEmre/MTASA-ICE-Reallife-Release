...
addEventHandler("setCasinoPreparationServer", getRootElement(), function()
	...
	if not Aktiondeaktivieren then
		infobox ( player, "Gerade ist es nicht möglich einen Casino Rob zu starten", 5000, 150, 0, 0 )
		return
	end
	...
	if casinoRobber == nil and not aktionlaeuft then
		...
		if int == 1 then
			if casinorobDonecaligulas[tonumber(MtxGetElementData(client, "fraktion"))] == nil or casinorobDonecaligulas[tonumber(MtxGetElementData(client, "fraktion"))] == false then
			else
				outputChatBox("Unbekannt: Sie haben uns doch heute schon \195\188berfallen, wir haben nichts mehr!", client, 255, 255, 255)
				return false
			end	
		elseif int == 10 then
			if casinorobDonefourdragons[tonumber(MtxGetElementData(client, "fraktion"))] == nil or casinorobDonefourdragons[tonumber(MtxGetElementData(client, "fraktion"))] == false then
			else
				outputChatBox("Unbekannt: Sie haben uns doch heute schon \195\188berfallen, wir haben nichts mehr!", client, 255, 255, 255)
				return false
			end	
		elseif int == 12 then
			if casinorobDonecasinofloor[tonumber(MtxGetElementData(client, "fraktion"))] == nil or casinorobDonecasinofloor[tonumber(MtxGetElementData(client, "fraktion"))] == false then
			else
				outputChatBox("Unbekannt: Sie haben uns doch heute schon \195\188berfallen, wir haben nichts mehr!", client, 255, 255, 255)
				return false
			end	
		...
		outputLog ( "Casinorob - "..getPlayerName(client).." - "..fraktionNames[MtxGetElementData ( client, "fraktion")], "aktion" )
		casinoRobberFraction = MtxGetElementData(client, "fraktion")
		casinoTimerMarker = setTimer(casinoRobEnded, 1000*60*14, 1, 4)
		casinoTimer = setTimer(casinoRobEnded, 1000*60*20, 1, 3)
		local x, y, z = getElementPosition(client)
		local robberFraction = MtxGetElementData(client, "fraktion")
		local players = getElementsByType("player")
		for i=1, #players do
			local player = players[i]
			local x1, y1, z1 = getElementPosition(player)
			local int1 = getElementInterior(player)
			if robberFraction == MtxGetElementData(player, "fraktion") then
		...
		if int == 1 then
			...
			casinorobDonecaligulas[tonumber(MtxGetElementData(client, "fraktion"))] = true
		...
		elseif int == 10 then
			...
			robbedFraction = 3
			casinorobDonefourdragons[tonumber(MtxGetElementData(client, "fraktion"))] = true
		...
		elseif int == 12 then
			...
			robbedFraction = 9
			casinorobDonecasinofloor[tonumber(MtxGetElementData(client, "fraktion"))] = true
...
function casinorobLootClick_func ( button, state, clickedElement )
	if MtxGetElementData(source, "fraktion") ~= casinoRobberFraction then return false end	
						...
						if getPlayerName(source) == player then
							...
							if clickedElement == thevehicle then
								...
								if casinorobErfolg == false then
									...
									-- "Yakuza" --
									if tonumber(MtxGetElementData ( casinoRobber, "fraktion" )) == 3 then
										abgabeMarker = createMarker(2815, 955.90002441406, 10.800000190735, "checkpoint", 6, 222, 0, 0, 255)
									-- "La Cosa Nostra" --
									elseif tonumber(MtxGetElementData ( casinoRobber, "fraktion" )) == 7 then
										abgabeMarker = createMarker(1041.5999755859, 2112.1999511719, 10.800000190735, "checkpoint", 6, 23, 23, 23)
									-- "Angels of Death" --
									elseif tonumber(MtxGetElementData ( casinoRobber, "fraktion" )) == 9 then
									...
									addEventHandler("onMarkerHit", abgabeMarker, function ( hitElement )
										if hitElement and getElementType(hitElement) == "vehicle" then
											if thevehicle == hitElement then
												...
												infobox_start_func ( getVehicleController(hitElement), "\nSteige aus dem Auto\naus und klick darauf,\num es auszuladen!", 5000, 0, 2000, 0 )
											end
										end
									end)
								end
...
function casinoRobEnded(success)
	...
	if success ~= 4 then
		...
		local players = getElementsByType("player")
		for i=1, #players do
			if MtxGetElementData(players[i], "fraktion") == casinoRobberFraction then
...
function getCasinorobLoot ( player, index )
	...
	elseif randomnumber >= 1 and randomnumber < 750 then
		...
		if itemnumber >= 93 then
			...
			MtxSetElementData ( player, "benzinkannister", tonumber ( MtxGetElementData ( player, "benzinkannister" ) ) + Benzinkanister )
		elseif itemnumber >= 65 then
			outputChatBox("Box "..tostring(index).." von "..tostring(anzahlafterobjects).." - "..Zigaretten.." Zigaretten", player)
			MtxSetElementData ( player, "zigaretten", MtxGetElementData ( player, "zigaretten" ) + Zigaretten )
		...
		elseif itemnumber >= 20 then
			...
			MtxSetElementData ( player, "mats", MtxGetElementData ( player, "mats" ) + Materialien )
...
function freezeDatShit (player)
	if MtxGetElementData(player, "fraktion") == casinoRobberFraction then
