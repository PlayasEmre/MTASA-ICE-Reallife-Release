				...
				if getPedWeaponSlot ( source ) ~= 0 then
					if getPedControlState ("aim_weapon") == true then
						...
						if getElementInterior(source) == 1 then
							if MtxGetElementData(source, "fraktion") == 3 or MtxGetElementData(source, "fraktion") == 7 or MtxGetElementData(source, "fraktion") == 9 or MtxGetElementData(source, "fraktion") == 12 or MtxGetElementData(source, "fraktion") == 13 or MtxGetElementData(source, "fraktion") == 14 then  
								...
								if getDistanceBetweenPoints2D (x, y, px, py) <= 10 then
									if target == caligulasPed[0] or target == caligulasPed[1] or target == caligulasPed[2] then
										thecasino = "caligulas"
										triggerServerEvent("setCasinoPreparationServer", localPlayer)
									else
						...
						elseif getElementInterior(source) == 10 then
							if MtxGetElementData(source, "fraktion") == 2 or MtxGetElementData(source, "fraktion") == 9 or MtxGetElementData(source, "fraktion") == 14 then   
						...
						elseif getElementInterior(source) == 12 then
							if MtxGetElementData(source, "fraktion") == 3 or MtxGetElementData(source, "fraktion") == 2 or MtxGetElementData(source, "fraktion") == 14 then  
...
addEventHandler ("casinorobOnMarkerHit", localPlayer, function(marker)
	progress = 0
	addEventHandler("onClientHUDRender", root, casinorobOnMarkerHitDXDraw)
end)

addEvent ("casinorobOnMarkerLeave", true)
addEventHandler ("casinorobOnMarkerLeave", localPlayer, function()
...
function casinorobOnMarkerHitDXDraw ()
	...
	if progress < aufbrechzeit then 
		...
		progress = 0
		triggerServerEvent("casinorobMarkerRobbed", localPlayer)	
	end
end
...
function playSoundOfRob_func (dacasino)
	...
	robSound = playSound(":"..getResourceName(getThisResource()).."/sounds/klingel.mp3", true)
...
addEventHandler("getCasinorobSpawnprotection", getRootElement(), function()
	removeEventHandler ( "onClientPlayerDamage", localPlayer, localPlayerCancelDamageInCasinoRob )
	removeEventHandler ( "onClientPlayerWeaponFire", localPlayer, stopLocalPlayerCancelDamageInCasinoRob )
	addEventHandler ( "onClientPlayerDamage", localPlayer, localPlayerCancelDamageInCasinoRob )
	addEventHandler ( "onClientPlayerWeaponFire", localPlayer, stopLocalPlayerCancelDamageInCasinoRob )
