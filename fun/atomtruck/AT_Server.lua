...
function ATBasis ( player )
	if not Aktiondeaktivieren then
	   infobox ( player, "Gerade ist es nicht möglich einen atomtruck zu starten", 5000, 150, 0, 0 )
	   return
	end
	if not getPedOccupiedVehicle ( player ) then
		if isOnDuty(player) then
			...
			if oldwanted <= 4 then
				...
				MtxSetElementData ( player, "wanteds", oldwanted + 2 )
			...
			elseif oldwanted == 5 then
				...
				MtxSetElementData ( player, "wanteds", oldwanted + 1 )
					...
					if isOnDuty(player) then
						if getRealTime().hour >= 12 then
							if money >= ATKaution then
								MtxSetElementData ( player, "money", money - ATKaution )
								...
								outputLog ( "Atomtruck - "..getPlayerName(player).." - "..fraktionNames[MtxGetElementData ( player, "fraktion")], "aktion" )
	...
	if getElementType( hitelement ) == "vehicle" then
		...
		if ( hitelement == ATTruck ) then
			if AbgabeATCheck then
				...
				MtxSetElementData ( driverScript, "money", money + ATKaution )
...
function resetAT ()
	...
	local minute = time.minute
		...
		if ( time.hour + 1 == 12 and time.minute == 0 ) then 
			...
			AbgabeATCheck = false
			ATStartable = true
			if isTimer ( checkTime ) then killTimer ( checkTime ) end
		...
		if ( ( tonumber ( MtxGetElementData ( player, "adminlvl" ) ) or 0 ) >= 1 ) then
			...
			elseif stringWithAllParameters == "" or stringWithAllParameters == " " or stringWithAllParameters == "  " then
				...
				if ( ATRunning == true ) then
					...
					local money = MtxGetElementData ( driverScript, "money" )
						MtxSetElementData ( ATTruck, "Atom", false )
						destroyElement ( ATTruck )
						destroyElement ( ATTrailer )
						MtxSetElementData ( driverScript, "money", money + ATKaution )
