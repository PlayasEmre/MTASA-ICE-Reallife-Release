...
addEventHandler( "onElementClicked", bank_door, 
	function ( _, state, player )
		...
		if not Aktiondeaktivieren then
			infobox ( player, "Gerade ist es nicht möglich einen Bankraub zu starten", 5000, 150, 0, 0 )
			return
		end
...
function giveBankKeyCard ( _, player )
	...
	if MtxGetElementData ( target, "wanteds" ) <= 3 then
		MtxSetElementData ( target, "wanteds", MtxGetElementData ( target, "wanteds" ) + 3 )
...
function exit_bank ( thePlayer, matchingDimension )
	...
	if MtxGetElementData (thePlayer, "gotdamoney") and not bereitsdraussen then
		MtxSetElementData ( bank_robber, "money", MtxGetElementData (bank_robber, "money" ) + 35000 )
...
function info_bank_timer ()
	...
	if bank_i == 0 then
		...
		if isElementWithinColShape ( bank_robber, bank_rob_col ) then
			...
			MtxSetElementData (bank_robber, "gotdamoney", true)
			setTimer ( function ( player )
				if MtxGetElementData (player, "gotdamoney") then
					if not bereitsdraussen then
						outputChatBox ( "Das Geld hat sich in Luft aufgelöst!", player, 200, 0, 0 )
					end
					MtxSetElementData (player, "gotdamoney", nil)
				end
...
function bank_col_leave (ele)
	...
	if ele == bank_robber then
		...
		if isTimer ( bank_timer ) then killTimer ( bank_timer ) end
		setTimer ( function() aktionlaeuft = false end, aktionpuffer, 1)
		robber_wasted()
...
function bank_rob ( player, _ )
	...
	if isEvil ( player ) and isElementWithinColShape ( player, bank_rob_col ) then
		...
		if bank_wird_ausgeraubt ~= 1 then
			...
			bank_robber = player
			bereitsdraussen = false
			outputLog ( "Bankraub - "..getPlayerName(player).." - "..fraktionNames[MtxGetElementData ( player, "fraktion")], "aktion" )
...
function nehmeGeldAb ( player, cmd, target )
	if isOnDuty ( player ) then
		if target then
			...
			if targetpl then
				if MtxGetElementData ( targetpl, "gotdamoney" ) then
					if bereitsdraussen then
						if MtxGetElementData ( targetpl, "money" ) >= 35000 then
							MtxSetElementData ( targetpl, "money", MtxGetElementData ( targetpl, "money" ) - 35000 )
						else 
							MtxSetElementData ( targetpl, "money", 0 )
						end
						MtxSetElementData (targetpl, "gotdamoney", false)
