...
function ram_func ( player )
	...
	if isOnDuty(player) and MtxGetElementData ( player, "rang" ) >= 3 then
...
function killawanze_func ( player, _, wich )
	...
	if isFBI(player) and MtxGetElementData ( player, "rang" ) >= 2 then
...
function wanze_func ( player )
	...
	if isFBI ( player ) then
		...
		MtxSetElementData ( player, "wanzen", true )
	else
