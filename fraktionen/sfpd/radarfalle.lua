...
-- Vorwaerts deklariert: onVehicleRespawnExplode_func benutzt beide bereits,
-- definiert werden sie aber erst weiter unten. Ohne diese Zeile zeigen die
-- Namen dort ins Leere (nil) und die removeEventHandler-Aufrufe entfernen
-- nichts - das Fahrzeug bliebe nach Respawn/Explosion dauerhaft gesperrt.
local stopFromExit, stopFromEnter
...
function stopFromExit(thePlayer, seat)
...
function stopFromEnter (thePlayer, seat)
	if allowExit == false and seat == 0 then
		cancelEvent()
	end
end
		...
		if id ~= 523 and id ~= 598 and id ~= 596 and id ~= 597 and id ~= 599 and id ~= 416 and id ~= 490 and id ~= 427 and id ~= 407 and id ~= 544 and id ~= 470 and id ~= 433 and id ~= 601 and id ~= 428 and id ~= 432 then  -- check if vehicles are not for government or emergency	
			...
			if 120 < kmh then
				...
				if driver and getElementType (driver) == "player" and not isOnDuty ( driver ) then
					local moneydriver = MtxGetElementData ( driver, "money" )
					local bankmoneydriver = MtxGetElementData ( driver, "bankmoney" )
					local price = (ticketTax*(kmh-120))
					if moneydriver >= price then
						MtxSetElementData ( driver, "money", moneydriver - price )
						outputChatBox("Du bist "..(kmh-120).." km/h zu schnell gefahren und musst "..price..""..Tables.waehrung.." bezahlen.", driver, 255, 0, 0 )	
					...
					elseif moneydriver + bankmoneydriver >= price then
						MtxSetElementData ( driver, "money", 0 )
						MtxSetElementData ( driver, "bankmoney", bankmoneydriver - ( price - moneydriver ) )
						outputChatBox("Du bist "..(kmh-120).." km/h zu schnell gefahren und musst "..price..""..Tables.waehrung.." bezahlen.", driver, 255, 0, 0 )	
						fadeCamera ( driver, false, 0.2, 255, 255, 255)
						setTimer ( fadeCamera, 500, 1, driver, true, 2 )
					else
						MtxSetElementData ( driver, "money", 0)
						MtxSetElementData ( driver, "bankmoney", 0)
					...
					MtxSetElementData ( ticketer, "money", MtxGetElementData ( ticketer, "money") + price )
					outputChatBox("Du hast "..getPlayerName(driver).." mit "..kmh.." km/h erwischt und kriegst "..price..""..Tables.waehrung.." dafür.", ticketer, 0, 255, 0 )
