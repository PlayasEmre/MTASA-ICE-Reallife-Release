...
function mv_func ( player )
	...
	if isCop(player) or isFBI(player) or isArmy(player) or isMedic(player) then
...
local function mv1_func ( player, dim )
	if dim then
		...
		if player and ( isCop(player) or isFBI(player) or isArmy(player) or isMedic(player) ) then
...
SFPDKerker2 = createObject ( 3089, 253.10000610352, 110.09999847412, 1003.5, 0, 0, 270 )
setElementInterior ( SFPDKerker2 , 10)
SFPDKerker2Moved = false
...
function mv_func ( player )
	...
	if isCop(player) or isFBI(player) or isArmy(player) or isMedic(player) then
		if getDistanceBetweenPoints3D ( 253.10000610352, 110.09999847412, 1003.5, getElementPosition ( player ) ) < 10 then
			if SFPDKerker2Moved == false then
				moveObject ( SFPDKerker2, 2000, 253.10000610352, 110.09999847412, 1003.5 )
				SFPDKerker2Moved = true
			else
				moveObject ( SFPDKerker2, 2000, 253.10000610352, 111.59999847412, 1003.5 )
				SFPDKerker2Moved = false
			end
		end
	end
...
SFPDKerker3 = createObject ( 3089, 239.69999694824, 116.5, 1003.5, 0, 0, 90.25 )
setElementInterior ( SFPDKerker3 , 10)
SFPDKerker3Moved = false

-- Wird von fraktionen/Anonymus/hacker.lua (Computer-Hack) aufgerufen, um bei
-- Erfolg die Kerker-Tueren direkt zu oeffnen (ohne isCop/isFBI/... Pruefung -
-- das ist ja der Sinn des Hacks). SFPDKerker2 wird bewusst ausgelassen: die
-- Variable wird oben zweimal mit unterschiedlichen Objekten belegt
-- (vorbestehender Bug, nicht Teil dieser Aenderung), das zugehoerige
-- Verhalten ist dadurch bereits mehrdeutig.
function vioOpenAllSFPDKerker ()
	if not SFPDKerker1Moved then
		moveObject ( SFPDKerker1, 2000, 227.77272033691, 132.4690246582, 1003.4538574219 )
		SFPDKerker1Moved = true
	end
	if not SFPDKerker3Moved then
		moveObject ( SFPDKerker3, 2000, 239.7001953125, 114.900390625, 1003.5 )
		SFPDKerker3Moved = true
	end
end

function mv_func2 ( player )

	if isCop(player) or isFBI(player) or isArmy(player) or isMedic(player) then
		if getDistanceBetweenPoints3D ( 239.69999694824, 116.5, 1003.5, getElementPosition ( player ) ) < 10 then
			if SFPDKerker3Moved == false then
				moveObject ( SFPDKerker3, 2000, 239.69999694824, 116.5, 1003.5 )
				SFPDKerker3Moved = true
			else
				moveObject ( SFPDKerker3, 2000, 239.7001953125, 114.900390625, 1003.5 )
				SFPDKerker3Moved = false
