...
newsPaperPath = ":"..Tables.servername.."/vio_stored_files/newspaper/newspaper.ice"
...
function newspaper_func ( player )
	...
	if MtxGetElementData ( player, "money" ) >= 10 then
		...
		if getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 ) <= 10 then
			MtxSetElementData ( player, "newspaper", true )
...
function readnewspaper_func ( player )
	...
	if MtxGetElementData ( player, "newspaper" ) then
...
function loadnewsPaper()
	...
	local newsPaper = fileOpen ( newsPaperPath, true )
...
function edit_func ( player )
	...
	if isReporter ( player ) and MtxGetElementData ( player, "rang" ) >= 3 then
...
function redoNewspaperServer_func ( text )
	...
	if source == client then
		MtxSetElementData ( getRootElement(), "newspaper", false )
		newsPaperText = text
