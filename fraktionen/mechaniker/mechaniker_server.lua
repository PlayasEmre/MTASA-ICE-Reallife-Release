-------------------------------------- Urheberrecht --------------- by [LA]Leyynen ------------------- 2012 ---------------------------------------------- Script by Noneatme ------

local debugmodus = false

createBlip(-2397.2683, -134.9355, 35.28888, 27, 2, 255, 0, 0, 255, 0, 200)
local duty_marker = createMarker(-2079.3227539063,-119.82526397705,39.709373474121, "corona", 1, 255, 0, 0, 150)
...
addCommandHandler("duty", function(thePlayer)
	if(isMechaniker(thePlayer)) and (isElementWithinMarker(thePlayer, duty_marker)) then
		...
		if(duty == true) then
			...
			MtxSetElementData(thePlayer, "repairkits", 5)
		end
	end
end)
...
function delete_mech_blip (player, thePlayer)
	...
	MtxSetElementData (player, "needMech", false)
end
...
addCommandHandler("repaircar", function(thePlayer, cmd)
	if(isMechaniker(thePlayer)) and (isEmergencyOnDuty(thePlayer)) then
		...
		local kits = tonumber(MtxGetElementData(thePlayer, "repairkits"))
		...
		if vehicle then
			...
			if kmh < 20 then
				...
				setTimer(function(thePlayer, vehicle)
					...
					MtxSetElementData(thePlayer, "repairkits", kits-1)
					fixVehicle(vehicle)
					if getVehicleOccupant(vehicle) ~= thePlayer then
