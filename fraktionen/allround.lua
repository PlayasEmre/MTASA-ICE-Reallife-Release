...
fraktionNames = {}
fraktionNames = {[0] = "Zivilisten", [1]="SFPD", [2]="Cosa Nostra", [3]="Triaden", [4]="Terror", [5]="SAN News", [6]="FBI", [7]="Los Aztecas", [8]="Bundeswehr", [9]="Angels of Death", [10]="Sanitäter", [11]="Mechaniker", [12]="Ballas", [13]="Grove",[14]="Anonymus",[15]="Fahrschule"}
...
function sendMSGToStateFactions (msg,r,g,b)
	...
	sendMSGForFaction ( msg, 2, r,g,b )
	sendMSGForFaction ( msg, 3, r,g,b )
	sendMSGForFaction ( msg, 4, r,g,b )
	sendMSGForFaction ( msg, 5, r,g,b )
	sendMSGForFaction ( msg, 6, r,g,b )
	sendMSGForFaction ( msg, 7, r,g,b )
	sendMSGForFaction ( msg, 8, r,g,b )
	sendMSGForFaction ( msg, 9, r,g,b )
	sendMSGForFaction ( msg, 10, r,g,b )
	sendMSGForFaction ( msg, 11, r,g,b )
	sendMSGForFaction ( msg, 12, r,g,b )
	sendMSGForFaction ( msg, 13, r,g,b )
	sendMSGForFaction ( msg, 14, r,g,b )
	sendMSGForFaction ( msg, 15, r,g,b )
end 
...
function isEvilCar ( car )
	if isMafiaCar( car ) or isTriadenCar( car ) or isTerrorCar ( car ) or isAztecasCar ( car ) or isBikerCar ( car ) or isBallasCar ( car ) or isGroveCar ( car ) or isAnonymusCar ( car ) then
		return true
	else
		return false
	end
...
function getPlayerFaction ( player )
	...
	local fac = MtxGetElementData ( player, "fraktion" )
...
function getPlayerRank ( player )
	...
	local ran = MtxGetElementData ( player, "rang" )
...
function isReporter(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 5 then return true else return false end
end

function isTerror(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 4 then return true else return false end
end

function isTriad(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 3 then return true else return false end
end

function isMafia(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 2 then return true else return false end
end

function isCop(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 1 then return true else return false end
end

function isFBI(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 6 then return true else return false end
end

function isAztecas(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 7 then return true else return false end
end

function isArmy(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 8 then return true else return false end
end

function isBiker(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 9 then return true else return false end
end

function isMedic(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 10 then return true else return false end
end

function isMechaniker(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 11 then return true else return false end
end

function isBallas(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 12 then return true else return false end
end

function isGrove(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 13 then return true else return false end
end

function isAnonymus(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 14 then return true else return false end
end

function isFahrschule(player)
	if tonumber(MtxGetElementData ( player, "fraktion" )) == 15 then return true else return false end
end
...
function isEvil(player)
	if isMafia(player) or isTriad(player) or isTerror(player) or isAztecas(player) or isBiker(player) or isBallas(player) or isGrove(player) or isAnonymus(player) then return true else return false end
end
...
function isFahrschuleDuty(player)
    if MtxGetElementData( player, "fahrschulonduty") then return true else return false end
end
...
function policeComputer ( presser, key, state )
	...
	if state == "down" and isOnDuty(presser) and isFederalCar ( getPedOccupiedVehicle ( presser ) ) and getElementModel ( getPedOccupiedVehicle ( presser ) ) ~= 520 then
		triggerClientEvent ( presser,"wantedcomputer", getRootElement() )
	end
			...
			if dim then
				if getElementType ( hit ) == "player" then
					if not getPedOccupiedVehicle ( hit ) then
						if needetFaction == 0 or MtxGetElementData ( hit, "fraktion" ) == needetFaction then
...
function createFactionVehicle ( model, x, y, z, rx, ry, rz, faction, c1, c2, c3, c4, numberplate )
	...
	MtxSetElementData ( veh, "sportmotor", ( faction == 10 and 3 or 2 ) )
	MtxSetElementData ( veh, "bremse", ( faction == 10 and 3 or 2 ) )
			...
			function ( player, seat )
				...
				if getPedOccupiedVehicleSeat ( player ) == 0 then
					...
					elseif getElementModel ( veh ) == 432 then
						...
						if MtxGetElementData ( player, "job" ) ~= "tankcommander" then
					...
					elseif getElementModel ( veh ) == 425 or getElementModel ( veh ) == 520 then
						...
						if MtxGetElementData ( player, "job" ) ~= "air" then
							opticExitVehicle ( player )
							outputChatBox ( "Du hast nicht die erforderliche Klasse!", player, 125, 0, 0 )
						end	
					...
					elseif getElementModel ( veh ) == 563 or getElementModel ( veh ) == 595 then
						...
						if MtxGetElementData ( player, "job" ) ~= "marine" and seat == 0 then
							opticExitVehicle ( player )
							outputChatBox ( "Du hast nicht die erforderliche Klasse!", player, 125, 0, 0 )
						else
			...
			if isEvil(player) or isOnDuty(player) then		
				if getVehicleOccupant( getPedOccupiedVehicle ( player ) ) ~= target and getPedOccupiedVehicleSeat ( target ) > 0 then				
					...
					if getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 ) <= 5 then
						...
						local boolean = not MtxGetElementData ( target, "tied" )
						MtxSetElementData ( target, "tied", boolean )
...
function fskin_func(player, cmd, number)
		...
		if target ~= false then
			...
			if getPlayerFaction( target ) == 0 then
				...
				if not isInGang ( getPlayerName ( target ) ) then
					...
					MtxSetElementData ( target, "fraktion", faction )
...
function getchangestate_func ( player, cmd, target )
	if playerUID[target] then
		if ( tonumber ( MtxGetElementData ( player, "adminlvl" ) ) or 0 ) >= 1 or ( tonumber ( MtxGetElementData ( player, "rang" ) ) or 0 ) >= 4 then
...
addEventHandler ( "fraktion_invite", root, function ( target )
	...
	if faction ~= 0 and rank >= 5 then
		...
		if isElement ( targetpl ) then
			if getPlayerFaction( targetpl ) == 0 then
				if not isInGang ( getPlayerName ( targetpl ) ) then	
					MtxSetElementData ( targetpl, "fraktion", faction )
					MtxSetElementData ( targetpl, "rang", 0 )
					MtxSetElementData ( targetpl, "FraktionenBetreten", MtxGetElementData ( targetpl, "FraktionenBetreten" ) + 1 )
		...
		elseif playerUID[target] then
			if tonumber ( (dbQueryCoro ( "SELECT ?? FROM ?? WHERE ??=?", "Fraktion", "userdata", "UID", playerUID[target] ))[1]["Fraktion"] ) == 0 then
	...
	if faction > 0 and rank >= 5 then
		...
		if isElement ( targetpl ) then		
			if faction == getPlayerFaction( targetpl ) and getPlayerRank ( targetpl ) <= 4 then			
				...
				MtxSetElementData ( targetpl, "skinid", model )
				MtxSetElementData ( targetpl, "rang", 0 )
				MtxSetElementData ( targetpl, "FraktionenVerlassen", MtxGetElementData ( targetpl, "FraktionenVerlassen" ) + 1 )
		...
		if targetrank < newrank then 		
			...
			if faction >= 1 and rank >= 4 and faction == targetfaction and rank > newrank then		
				if newrank < 5 and newrank >= 0 then			
					...
					MtxSetElementData ( targetpl, "rang", newrank )
...
function selbstUninvite ( player )
	...
	if faction > 0 and rank < 5 then
		...
		MtxSetElementData ( player, "skinid", model )
		...
		MtxSetElementData ( player, "FraktionenVerlassen", MtxGetElementData ( player, "FraktionenVerlassen" ) + 1 )
		for playeritem, _ in pairs ( fraktionMembers[faction] ) do
			triggerClientEvent ( playeritem, "syncPlayerList", player, fraktionMemberList[faction], fraktionMemberListInvite[faction] )
		end
...
function startFrespawn ( player )
	...
	local frak = MtxGetElementData ( player, "fraktion" )
	if frak == 1 then
		for veh, _ in pairs ( factionVehicles[frak] ) do
			if not getVehicleOccupant ( veh ) then
				respawnVehicle ( veh )
				setElementFrozen ( veh, true )
				setElementData(veh,"totalschaden",0)
				setElementData(veh,"Beschlagnahmt",0)
			end
		end
	elseif frak == 2 then
		for veh, _ in pairs ( factionVehicles[frak] ) do
	...
	elseif frak == 14 then
		for veh, _ in pairs (factionVehicles[14]) do
			if not getVehicleOccupant ( veh ) and getElementDimension ( veh ) ~= diegangwardimension then
				respawnVehicle ( veh )
				setElementFrozen ( veh, true )
				setElementData(veh,"totalschaden",0)
...
function allowFSkinFunction ( player )
	if MtxGetElementData ( player, "fraktion" ) == 1 and MtxGetElementData ( player, "rang" ) >= 4 then
