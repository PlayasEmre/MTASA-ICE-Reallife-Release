...
function blackListKillCheck ( player, killer, weapon )
	local killerFaction = MtxGetElementData ( killer, "fraktion" )
	local name = getPlayerName ( player )
	if validBlackListFactions[killerFaction] then
		if blacklistPlayers[killerFaction][name] then
			local prizeMoney = 200
			local prizeText = "Du erhälst 200 "..Tables.waehrung..""
			if factionBlackListGuns[killerFaction] == weapon then
				prizeText = prizeText.." + 100 "..Tables.waehrung.." wegen der verwendeten Waffe."
...
function blacklist_func ( player, cmd, add, target, ... )
	if not add then
		...
		if validBlackListFactions[MtxGetElementData ( player, "fraktion" )] then
			if add == "add" then
