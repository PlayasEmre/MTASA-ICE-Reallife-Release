...
-- Einkommen & Bonus --
local minanzahlfuerbonus = 2   							-- Wieviele Spieler mindestens online sein müssen für die Boni
local hoherbonus = 5    								-- Der höhere Bonus Mats/Drogen je Ganggebiet
local niedrigerbonus = 200    							-- Der niedrigere Bonus Mats/Drogen je Ganggebiet 
...
local maxanzahlanattacks = 300   						-- Maximale Anzahl an Attacks pro Tag
minanzahlangwteilnehmern = 0						-- Mindestanzahl an Spielern in beiden Frakis für Attack
diegangwardimension = 14    						-- Die Dimension, in der der Gangwar stattfinden soll
local minutenzumgwsieg = 15   							-- Minuten zu Halten, um den Gangwar als Angreifer zu gewinnen
local vorbereitungszeit = 2  							-- Minuten zur Vorbereitung vor dem richtigen Gangwar
local reichweitezumtk = 20 								-- Mindestreichweite zum TK, um drin zu sein - ACHTUNG: Clientseitig auch ändern!
local minutennachgwwaffenbekommen = 0   				-- Nach wievielen Minuten nach dem Gangwar man die Waffen zurück bekommen soll. Wenn man nach dem GW sofort am Spawnpunkt spawnt am besten 0, andernfalls 2
...
local gebewaffenvorgw = true     						-- Ob man die Waffen von der Tabelle givenWeapons vor dem GW bekommen soll
		...
		if getFactionMembersOnline ( index ) >= minanzahlfuerbonus then
			if tables.mats + tables.drogen + tables.geld > 0 then
				for playeritem, _ in pairs ( fraktionMembers[index] ) do
					...
					outputChatBox ( "#C8C800Geld: #35A327"..tables.geld..""..Tables.waehrung.."", playeritem, 0, 0, 0, true )
...
function gangPickupHit ( hit )
	...
	outputChatBox ( "Einnahmen/Stunde: "..gangPickup[id]["einnahmen"]..""..Tables.waehrung.." und "..bonus, hit, 125, 125, 200 )
	local fraktion = MtxGetElementData ( hit, "fraktion" )
	if gangAttacks[fraktion] then
		if MtxGetElementData ( hit, "rang" ) >= 2 then
...
function gangattack_func ( player )
	if gangAreaAttackbar then
		if not gangAreaUnderPreparation and not gangAreaUnderAttack then
			...
			local attackerfrac = MtxGetElementData ( player, "fraktion" )
			if gangAttacks[attackerfrac] and MtxGetElementData ( player, "rang" ) >= mindestrangzumattack then
...
function startGangAreaAttack ( )
	...
	for key, thePlayer in pairs ( getElementsWithinColShape ( gangPickup[validID]["colshape"], "player" ) ) do
		if MtxGetElementData ( thePlayer, "fraktion" ) == gangData.attackerfrac then
...
function areaEroberungsCheck ( )
	...
	for index, player in pairs ( getElementsWithinColShape ( gangPickup[validID]["tkcolshape"], "player" ) ) do
		if MtxGetElementData ( player, "fraktion" ) == gangData.attackerfrac then	
...
function gangwarDefenderGewonnen ()
	...
	for playeritem, tables in pairs ( playerData ) do
		...
		if isElement ( player ) and MtxGetElementData ( player, "fraktion" ) == gangData.attackerfrac then
			MtxSetElementData ( player, "AnzahlGangwarsVerloren", ( MtxGetElementData ( player, "AnzahlGangwarsVerloren" ) or 0 ) + 1 )
		elseif isElement ( player ) then
			MtxSetElementData ( player, "AnzahlGangwarsGewonnen", ( MtxGetElementData ( player, "AnzahlGangwarsGewonnen" ) or 0 ) + 1 )
		end
	end
	zuruckGebenTimer = setTimer ( function()
		for playeritem, tables in pairs ( playerData ) do
			...
			if isElement ( player ) then
				...
				infobox ( player, "Bonus:\n".. ( math.floor ( tables["damage"] ) * geldprodamageimgw ) + ( tables["kills"] * geldprokillimgw ).. ""..Tables.waehrung.."", 4000, 0, 120, 0 )
...
local function dontLetOthersUseIt ( player )
	if MtxGetElementData ( player, "fraktion" ) ~= gangData.ownerfrac then
		cancelEvent()
	end
end
...
function setBurritosBackInGangwar ( player )
	if gangAreaUnderPreparation and MtxGetElementData ( player, "fraktion" ) == gangData.attackerfrac then
		local x, y, z = getElementPosition ( gangPickup[validID]["pickup"] )
...
function gangwarPreparation ( player )
	...
	elseif isPedInVehicle ( player ) then
		...
		if seat == 0 then
			...
			if factionVehicles[MtxGetElementData ( player, "fraktion")][vehicle] then
				isfracvehicle = true
				spielerSitztImFahrzeug[player] = vehicle
				spielerSitztImFahrzeugSitz[player] = 0
				for seat, occupant in pairs ( getVehicleOccupants ( vehicle ) ) do
					if seat ~= 0 then
						if MtxGetElementData ( occupant, "fraktion" ) == MtxGetElementData ( player, "fraktion" ) then
							if MtxGetElementData ( occupant, "fraktion" ) == gangData.ownerfrac then
							...
							elseif MtxGetElementData ( occupant, "fraktion" ) == gangData.attackerfrac then
	...
	if not playerData[pname] then
		...
		MtxSetElementData ( player, "AnzahlGangwars", ( MtxGetElementData ( player, "AnzahlGangwars" ) or 0 ) + 1 )
	end
	playerData[pname]["imGW"] = true
	playerData[pname]["hatDMGbekommen"] = false
	playerData[pname]["damage"] = 0
	playerData[pname]["kills"] = 0
	playerData[pname]["attacker"] = ( MtxGetElementData ( player, "fraktion" ) == gangData.attackerfrac )
...
function gangwarDeathfunc ( attacker )
	...
	if playerData[pname] and playerData[pname]["imGW"] then
		...
		local frac = MtxGetElementData(client,"fraktion")
		...
		local isattacker = MtxGetElementData ( client, "fraktion" ) == gangData.attackerfrac
		if gangAreaUnderPreparation then
			if isattacker then
				gangData.attacker = gangData.attacker - 1
		...
		elseif gangAreaUnderAttack then
			...
			if isElement ( attacker ) and getElementType ( attacker ) == "player" and attackername and playerData[attackername] and playerData[attackername]["imGW"] and MtxGetElementData ( attacker, "fraktion" ) ~= MtxGetElementData ( client, "fraktion" ) then
				playerData[attackername]["kills"] = playerData[attackername]["kills"] + 1
				MtxSetElementData ( attacker, "GangwarKills", ( MtxGetElementData ( attacker, "GangwarKills" ) or 0 ) + 1 )
				triggerClientEvent ( attacker, "rechneKillAn", attacker, playerData[attackername]["kills"] )
				if MtxGetElementData ( client, "lasthp" ) then
					triggerClientEvent ( attacker, "rechneDMGAn", attacker, MtxGetElementData ( client, "lasthp" ) )
					playerData[attackername]["damage"] = playerData[attackername]["damage"] + MtxGetElementData ( client, "lasthp" )
					MtxSetElementData ( attacker, "GangwarDamageGemacht", ( MtxGetElementData ( attacker, "GangwarDamageGemacht" ) or 0 ) + MtxGetElementData ( client, "lasthp" ) )
					MtxSetElementData ( client, "GangwarDamageBekommen", ( MtxGetElementData ( client, "GangwarDamageBekommen" ) or 0 ) + MtxGetElementData ( client, "lasthp" ) )
				end
		...
		MtxSetElementData ( client, "GangwarTode", ( MtxGetElementData ( client, "GangwarTode" ) or 0 ) + 1 )
	end		
end
...
function gangwarOfflinefunc ( )
	...
	if playerData[pname] then
		...
		if isElement ( teamBlips[MtxGetElementData(source,"fraktion")][source] ) then
			destroyElement ( teamBlips[MtxGetElementData(source,"fraktion")][source] )
		end
		local isattacker = MtxGetElementData ( source, "fraktion" ) == gangData.attackerfrac
		if gangAreaUnderPreparation then
			if isattacker then
				gangData.attacker = gangData.attacker - 1
				gangData.attackeralive = gangData.attackeralive - 1
...
function spawnPlayerAfterGangwar ( player )
	if isElement ( player ) then
		...
		spawnPlayer ( player, MtxGetElementData ( player, "spawnpos_x" ), MtxGetElementData ( player, "spawnpos_y" ), MtxGetElementData ( player, "spawnpos_z" ), (MtxGetElementData ( player, "spawnrot_x" ) or 0), skinid, MtxGetElementData ( player, "spawnint" ), MtxGetElementData ( player, "spawndim" ), team ) 
	end
end
...
function allowAttack ( player, cmd, targetfrac )
	local eigenefrac = MtxGetElementData ( player, "fraktion" )
	if gangAttacks[eigenefrac] and MtxGetElementData ( player, "rang" ) >= minrangzumattackgeben then
		if targetfrac then
			local fraczahl = nil
			local fracname = ""
			...
			if not gangAllow[fraczahl..eigenefrac] then
				...
				if isTimer ( allowTimer[fraczahl..eigenefrac] ) then killTimer ( allowTimer[fraczahl..eigenefrac] ) end
				gangAllow[fraczahl..eigenefrac] = nil
...
function gangwarAllowToJoinPluzBattlexChampStinkt ( player, cmd, targetpl )
	if gangAreaUnderAttack then
		if MtxGetElementData ( player, "fraktion" ) == gangData.attackerfrac or MtxGetElementData ( player, "fraktion" ) == gangData.ownerfrac then
			if MtxGetElementData ( player, "rang" ) >= minrangzumjoinerlaubnisgeben then
				if targetpl and findPlayerByName ( targetpl ) then
					local targetpl = findPlayerByName ( targetpl )
					if MtxGetElementData ( targetpl, "fraktion" ) == gangData.attackerfrac or MtxGetElementData ( targetpl, "fraktion" ) == gangData.ownerfrac then
						if MtxGetElementData ( targetpl, "fraktion" ) ~= MtxGetElementData ( player, "fraktion" ) then
