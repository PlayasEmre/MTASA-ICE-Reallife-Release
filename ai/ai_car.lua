...
function createNewCarAI ()
	...
	if not curAiCarSpots[carspot]["bool"] then
		...
		MtxSetElementData ( curAiCarSpots[carspot]["bot"], "id", carspot )
		MtxSetElementData ( curAiCarSpots[carspot]["bot"], "typ", "car" )
		MtxSetElementData ( curAiCarSpots[carspot]["bot"], "veh", veh )
		MtxSetElementData ( curAiCarSpots[carspot]["bot"], "clickPed", true )
		
		MtxSetElementData ( veh, "name", name )
		MtxSetElementData ( veh, "price", price )
		...
		addEventHandler ( "onPedWasted", curAiCarSpots[carspot]["bot"],
			function ( ammo, killer )
				...
				local data = MtxGetElementData ( source, "id" )
				curAiCarSpots[data]["bool"] = false
				
				if killer then
					MtxSetElementData ( killer, "wanteds", MtxGetElementData ( killer, "wanteds" ) + 2 )
					if MtxGetElementData ( killer, "wanteds" ) > 6 then
						MtxSetElementData ( killer, "wanteds", 6 )
...
function buyCarFromAI ( player, car, ped )
	...
	local price = tonumber ( MtxGetElementData ( car, "price" ) )
	if price <= MtxGetElementData ( player, "money" ) then
		...
		if bool then
			...
			local data = MtxGetElementData ( ped, "id" )
			curAiCarSpots[data]["bool"] = false
		end
	else
