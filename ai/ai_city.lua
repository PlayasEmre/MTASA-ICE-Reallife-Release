...
function createNewAI ()
	...
	while true do
		...
		elseif not usedAIPositions[rnd] then
			...
			MtxSetElementData ( ped, "typ", typ )
			MtxSetElementData ( ped, "clickPed", true )
			MtxSetElementData ( ped, "pos", rnd )
			
			if typ == "wdealer" then
				local gun = generateWeapon()
				local price = math.floor ( aiGunPrices[gun] * math.random ( 7, 15 ) / 10 )
				MtxSetElementData ( ped, "item", aiGuns[gun] )
				MtxSetElementData ( ped, "price", price )
			elseif typ == "sdealer" then
				local rnd = math.random ( 10, 100 )
				local price = math.floor ( rnd * math.random ( 5, 15 ) / 10 * 40 )
				MtxSetElementData ( ped, "item", rnd )
				MtxSetElementData ( ped, "price", price )
			...
			elseif typ == "gunbuyer" then
				...
				if rnd == 1 then
					...
					MtxSetElementData ( ped, "item", 22 )
					MtxSetElementData ( ped, "price", price )
				elseif rnd == 2 then
					local price = math.random ( 75, 300 )
					MtxSetElementData ( ped, "item", 4 )
					MtxSetElementData ( ped, "price", price )
				else
					local price = math.random ( 250, 750 )
					MtxSetElementData ( ped, "item", 30 )
					MtxSetElementData ( ped, "price", price )
				end
			end
			...
			addEventHandler ( "onPedWasted", ped,
				function ( ammo, killer )
					...
					if killer then
						-- setPlayerWantedLevel akzeptiert NUR 0-6 - vorher wurde nur nach
						-- oben gedeckelt, nicht nach unten. War "wanteds" durch irgendeine
						-- andere Stelle im Skript negativ oder ungueltig geworden, flog
						-- hier ein Laufzeitfehler ("Valid wanted levels are between 0 and
						-- 6 inclusive"), weil +2 dann immer noch ausserhalb 0-6 lag.
						local wanteds = ( tonumber ( MtxGetElementData ( killer, "wanteds" ) ) or 0 ) + 2
						wanteds = math.max ( 0, math.min ( 6, math.floor ( wanteds ) ) )
						MtxSetElementData ( killer, "wanteds", wanteds )
						setPlayerWantedLevel ( killer, wanteds )
...
function agreeWithPed_func ()
	...
	local player = source
	local ped = MtxGetElementData ( player, "curclicked" )
	if isElement ( ped ) then
		if not isPedDead ( ped ) then
			local typ = MtxGetElementData ( ped, "typ" )
			if typ == "car" then
				local car = curAiCarSpots[MtxGetElementData ( ped, "id" )]["car"]
				buyCarFromAI ( player, car, ped )
			else
				local item = MtxGetElementData ( ped, "item" )
				local price = MtxGetElementData ( ped, "price" )
				if typ == "bum" then
					local sucess = false
					for i = 1, 3 do
						if MtxGetElementData ( player, "food"..i ) == 3 then
							sucess = true
							MtxSetElementData ( player, "food"..i, 0 )
							break
						end
					end
					if not sucess then
						...
						outputChatBox ( "Aus Dank hat dir der Obdachlose 100 "..Tables.waehrung.." Geschenkt - woher auch immer er die hatte...", player, 0, 125, 0 )
				...
				elseif typ == "wdealer" then
					...
					if money >= price then
						if MtxGetElementData ( player, "gunlicense" ) == 0 then
				...
				elseif typ == "sdealer" then
					...
					if money >= price then
						...
						MtxSetElementData ( player, "flowerseeds", MtxGetElementData ( player, "flowerseeds" ) + item )
				...
				elseif typ == "dealer" then
					...
					if money >= price then
						MtxSetElementData ( player, "money", money - price )
						MtxSetElementData ( player, "drugs", MtxGetElementData ( player, "drugs" ) + item )
