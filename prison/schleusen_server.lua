...
for i, obj in pairs ( schleusen_keypads ) do
	...
	MtxSetElementData ( obj, "SchleusenNummer", count )
...
-- Erfolg alle Schleusen auf einmal zu oeffnen, statt einzeln pro Keypad.
function vioOpenAllSchleusen ()
	for i = 1, 4 do
		if schleusen_status[i] == "zu" then
			vioToggleSchleuse ( i )
		end
	end
end
...
function outputClick ( mouseButton, buttonState, ele )
	...
	if buttonState == "up" then
			...
			if not MtxGetElementData( ele, "SchleusenNummer" ) or MtxGetElementData( ele, "SchleusenNummer" ) == nil then
				return
			end
			...
			local schleuse = tonumber(MtxGetElementData( ele, "SchleusenNummer" ))
