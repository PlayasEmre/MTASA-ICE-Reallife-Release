...
function arrest_func ( player )
	local arrest_wanteds = tonumber(MtxGetElementData( player, "wanteds" ))
	local time
	local fine
	local kaution
	local targetfrac = MtxGetElementData( player, "fraktion" )
	local arrester = nil
	...
	if isElement ( veh ) then
		...
		MtxSetElementData ( player, "arrester", arrester )
		removePedFromVehicle ( player ) 	
	end

	local boolean = not MtxGetElementData ( player, "tied" )
	if boolean then 
		local fix = "ent" 
		MtxSetElementData ( player, "tied", boolean )
...
function arrestPlayer ( officer, player, time, fine, bail )
	...
	if fine > money then		
		MtxSetElementData ( player, "money", 300 )
	else
		MtxSetElementData ( player, "money", money - fine )
	end
	...
	if officer then
		MtxSetElementData ( officer, "boni", MtxGetElementData ( officer, "boni" ) + MtxGetElementData ( player, "wanteds" ) * wantedarrestmoney )
	end
	...
	if bail == 0 then
		...
		MtxSetElementData ( player, "bail", bail )
		if officer then
	...
	if officer then
		...
		MtxSetElementData ( officer, "AnzahlEingeknastet", MtxGetElementData ( officer, "AnzahlEingeknastet" ) + 1 )
	else
...
function bail_func ( player )
	if MtxGetElementData ( player, "jailtime" ) == 0 then
		...
		if MtxGetElementData ( player, "bail" ) == 0 or MtxGetElementData ( player, "prison" ) > 0 then
			triggerClientEvent ( player, "infobox_start", getRootElement(), "\nDu hast\nkeine Kaution!", 5000, 125, 0, 0 )
		else
			local bail = MtxGetElementData ( player, "bail" )
			local money = MtxGetElementData ( player, "money" )
			if bail <= money then
				MtxSetElementData ( player, "money", money - bail )
				freePlayerFromJail ( player )
				if isElement ( MtxGetElementData ( player, "arrester" ) ) then
					local arrester = MtxGetElementData ( player, "arrester" )
					MtxSetElementData ( player, "arrester", nil )
					MtxSetElementData ( arrester, "money", MtxGetElementData ( arrester, "money" ) + bail/4 )
				...
				if bail <= money + bankmoney then
					...
					MtxSetElementData ( player, "money", 300 )
					MtxSetElementData ( player, "bankmoney", bankmoney - rest )
					if isElement ( MtxGetElementData ( player, "arrester" ) ) then
						local arrester = MtxGetElementData ( player, "arrester" )
						MtxSetElementData ( player, "arrester", nil )
						MtxSetElementData ( arrester, "money", MtxGetElementData ( arrester, "money" ) + bail/4 )
...
function freePlayerFromJail ( player )
	...
	if MtxGetElementData ( player, "heaventime" ) == 0 then
...
function jailtime_func ( player )
	local jailtime = MtxGetElementData ( player, "jailtime" )
	if jailtime == 0 then
		local prisontime = MtxGetElementData ( player, "prison" )
			...
			if getElementInterior ( player ) == 10 then
				if stellenAn then
					...
					if getDistanceBetweenPoints3D ( x, y, z, x1, y1, z1 ) <= 10 then
						...
						local wanteds = tonumber(MtxGetElementData(player, "wanteds"))
						local targetfrac = tonumber(MtxGetElementData(player, "fraktion"))
...
local function prisonCarHit ( hitElement, dim )
	if getElementType ( hitElement ) == "player" and dim then
		if MtxGetElementData ( hitElement, "wanteds" ) > 0 then
