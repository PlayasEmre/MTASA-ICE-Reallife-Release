...
-- Global: jobs/allround.lua blendet den Blip beim /quitjob aus.
fischerblip = createBlip ( -1724.9904785156, 1461.3231201172, 7, 58, 2, 255, 255, 0, 255, 0, 200 )
...
function VehicleFischerEnter ( player, seat )
	if MtxGetElementData ( player, "job" ) ~= "fischer" then
		opticExitVehicle ( player )
		triggerClientEvent ( player, "infobox_start", player, "\n\nDu bist\nkein Fischer!", 5000, 125, 0, 0 )
	elseif MtxGetElementData ( player, "job" ) == "fischer" then
		triggerClientEvent ( player, "startfishing", player )
		local t = getRealTime()
		local minutes = t.minute 
		local hour = t.hour
		MtxSetElementData ( player, "fishingStartedM", minutes )
		MtxSetElementData ( player, "fishingStartedH", hour )
	end
end
...
function endfishing_func ( fische )
	...
	if source == client then
		...
		local minutes = t.minute 
		local hour = t.hour
		local dist = getDistanceBetweenMinutes ( MtxGetElementData ( source, "fishingStartedM" ), MtxGetElementData ( source, "fishingStartedH" ), minutes, hour )
		if dist * 7 > fische then
		-- ANTICHEAT --
			if fische >= 1 then
				local einnahmen = fishprice*fische
				local einnahmen = math.floor(einnahmen)*20
				triggerClientEvent ( source, "infobox_start", source, "Du hast "..fische.."\nFische gefangen und\nerhälst "..einnahmen.." "..Tables.waehrung.."\ndafür!", 15000, 125, 0, 0 )
				...
				if MtxGetElementData ( source, "angler_achiev" ) ~= "done" then																							-- Achiev: Angler
					...
					if MtxGetElementData ( source, "angler_achiev" ) >= 200 then																							-- Achiev: Angler
