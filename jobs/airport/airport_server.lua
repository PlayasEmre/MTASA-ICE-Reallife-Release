...
function cancelAirportMission ( veh )
	if MtxGetElementData ( source, "isinairportmission" ) then
		if flughafenModels [ getElementModel ( veh ) ] then
			...
			MtxSetElementData ( source, "isinairportmission", false )
...
function airportjobiconHit ( player )
	if MtxGetElementData ( player, "job" ) == "airport" and not getPedOccupiedVehicle ( player ) then
		local dim = getFreeDimension ()
		MtxSetElementData ( player, "jobDimension", dim )
...
function airportjobDimFix_func ( )
	...
	local player = client
	MtxSetElementData ( player, "isinairportmission", true )
	local pname = getPlayerName ( player )
	local dim = MtxGetElementData ( player, "jobDimension" )
...
function airportJobInsektenvernichter_func ( )
	...
	MtxSetElementData ( player, "airportMissionVeh", _G["Cropduster"..pname] )
...
function airportMissionCrash ()
	local veh = MtxGetElementData ( source, "airportMissionVeh" )
	if veh then
...
function cropdusterMissionComplete_func ( )
	...
	if MtxGetElementData ( player, "isinairportmission" ) then	
		...
		setElementDimension ( player, 0 )
		infobox ( player, "Auftrag abgeschlossen!\nDu erhälst 250 "..Tables.waehrung.."!", 5000, 0, 125, 0 )
		increaseAirportLevel ( player, 2 )
		local pmoney = MtxGetElementData ( player, "money" )
		MtxSetElementData ( player, "money", pmoney + 250 )
		MtxSetElementData(player,"coins",tonumber(MtxGetElementData(player,"coins"))+20)
		outputChatBox("Du hast 20 Coins erhalten!", player, 255, 255, 255)
		setElementPosition ( player, -1417.6236572266, -302.36517333984, 5.8523507118225 )
	end
...
function increaseAirportLevel ( player, lvl )
	...
	local airportlvl = tonumber ( MtxGetElementData ( player, "airportlvl" ) )
	if airportlvl < 50 then
		if airportlvl + lvl >= 50 then
			MtxSetElementData ( player, "airportlvl", 50 )
			...
			MtxSetElementData ( player, "bonuspoints", tonumber(MtxGetElementData ( player, "bonuspoints" )) + 25 )
		else
			MtxSetElementData ( player, "airportlvl", airportlvl + lvl )
...
function airportJobFlight_func ( veh, x, y, z, rot )
	...
	MtxSetElementData ( player, "airportMissionVeh", _G["Plane"..pname] )
	removeEventHandler ( "onPlayerQuit", player, airportMissionCrash )
	addEventHandler ( "onPlayerQuit", player, airportMissionCrash )
end
...
function airportJobFreightFinished_func ( vehid )
	...
	if MtxGetElementData ( player, "isinairportmission" ) then
		...
		infobox ( player, "Auftrag abgeschlossen!\nDu erhälst "..earned.." "..Tables.waehrung.."!", 5000, 0, 125, 0 )
		increaseAirportLevel ( player, 2 )
		local pmoney = MtxGetElementData ( player, "money" )
		MtxSetElementData ( player, "money", pmoney + earned )
		MtxSetElementData(player,"coins",tonumber(MtxGetElementData(player,"coins"))+20)
		outputChatBox("Du hast 20 Coins erhalten!", player, 255, 255, 255)
		setElementPosition ( player, -1417.6236572266, -302.36517333984, 5.8523507118225 )
	end
