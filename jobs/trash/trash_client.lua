...
function showStreetCleanGUI_func ()
	...
	if gWindow["cleanJob"] then
		dgsSetVisible ( gWindow["cleanJob"], true )
		dgsBringToFront(gWindow["cleanJob"])
	else
		gWindow["cleanJob"] = dgsCreateWindow(screenwidth/2-221/2,screenheight/2-419/2,221,419,"Strassenreinigung",false, nil, nil, nil, nil, nil, tocolor(16,29,61,255), nil, true)
		dgsBringToFront(gWindow["cleanJob"])
		dgsWindowSetMovable(gWindow["cleanJob"],false)
		dgsWindowSetSizable(gWindow["cleanJob"],false)
		gLabel[1] = dgsCreateLabel(9,25,160,17,"Deine aktuellen Jobpunkte:",false,gWindow["cleanJob"])
		...
		gLabel[2] = dgsCreateLabel(96,44,25,16,"0",false,gWindow["cleanJob"])
		...
		gButton[1] = dgsCreateButton(50,141,111,41,"Als Muellsammler arbeiten",false,gWindow["cleanJob"], _, _, _, _, _, _, tocolor(46,110,255,255), tocolor(123,163,255,255), tocolor(24,74,199,255))
		dgsSetFont(gButton[1],"default-bold")
		gLabel[3] = dgsCreateLabel(11,75,190,60,"Bewege dich über das Gelände\nund sammle den Müll ein -\npro eingesammeltem Teil Müll\nbekommst du 5$ Bezahlung.",false,gWindow["cleanJob"])
		...
		gLabel[4] = dgsCreateLabel(40,190,145,56,"Fahr durch die Stadt und\nbeseitige den Müll -\npro Müllhaufen erhälst\ndu 50$.",false,gWindow["cleanJob"])
		...
		gButton[2] = dgsCreateButton(51,253,111,41,"Als Strassen-\nreiniger arbeiten",false,gWindow["cleanJob"], _, _, _, _, _, _, tocolor(46,110,255,255), tocolor(123,163,255,255), tocolor(24,74,199,255))
		dgsSetFont(gButton[2],"default-bold")
		gLabel[5] = dgsCreateLabel(24,310,180,44,"Fahr mit dem Müllwagen durch\ndie Stadt und leere die Tonnen.\nPro Tonne erhälst du 80$.",false,gWindow["cleanJob"])
		...
		gButton[3] = dgsCreateButton(53,356,111,41,"Als Müllwagen-\nfahrer arbeiten",false,gWindow["cleanJob"], _, _, _, _, _, _, tocolor(46,110,255,255), tocolor(123,163,255,255), tocolor(24,74,199,255))
		dgsSetFont(gButton[3],"default-bold")
		gLabel[6] = dgsCreateLabel(59,295,95,14,"* ab 50 Jobpunkten",false,gWindow["cleanJob"])
		dgsLabelSetColor(gLabel[6],255,255,255,255)
		dgsLabelSetVerticalAlign(gLabel[6],"top")
		dgsLabelSetHorizontalAlign(gLabel[6],"left",false)
		dgsSetFont(gLabel[6],"arial")
		gLabel[7] = dgsCreateLabel(63,400,95,14,"* ab 250 Jobpunkten",false,gWindow["cleanJob"])
		dgsLabelSetColor(gLabel[7],255,255,255,255)
		dgsLabelSetVerticalAlign(gLabel[7],"top")
		dgsLabelSetHorizontalAlign(gLabel[7],"left",false)
		dgsSetFont(gLabel[7],"arial")
		gButton[4] = dgsCreateButton(194,22,17,17,"x",false,gWindow["cleanJob"], _, _, _, _, _, _, tocolor(46,110,255,255), tocolor(123,163,255,255), tocolor(24,74,199,255))

		addEventHandler ( "onDgsMouseClickUp", gButton[4],
			function(btn)
				if btn ~= "left" then return end
				dgsSetVisible ( gWindow["cleanJob"], false )
				showCursor ( false )
		...
		addEventHandler ( "onDgsMouseClickUp", gButton[3],
			function(btn)
				...
				if vioClientGetElementData ( "streetCleanPoints" ) >= 250 then
					...
					infobox_start_func("\nDu benötigst mind.\n250 Jobpunkte, um\nals Müllwagenfahrer\narbeiten zu koennen.",7500)
				end
			end,
		false )
	end
	dgsSetText ( gLabel[2], tostring ( vioClientGetElementData ( "streetCleanPoints" ) ) )
end
...
function startFoodJob ()
	...
	infobox_start_func("Sammel den Müll\nein, der auf dem\nRadar markiert ist -\ntippe /endjob, um den\nJob abzubrechen.",7500)
...
function startTrashmasterJob ()
	...
	infobox_start_func("Leere die Tonnen,\ndie auf dem\nRadar markiert sind -\ntippe /endjob, um den\nJob abzubrechen.",7500)
