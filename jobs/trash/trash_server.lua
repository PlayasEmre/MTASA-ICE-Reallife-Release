...
function trashJobIconHit ( player )
	...
	if MtxGetElementData ( player, "job" ) == "streetclean" and not getPedOccupiedVehicle ( player ) then
...
function trashCheckOccus ( veh )
	...
	if not getVehicleOccupant ( veh ) then
		...
		local eledata = MtxGetElementData ( veh, "TrashDriverVehicle" )
...
function createStreetCleanVeh_func ( veh )
	...
	if veh == 574 or veh == 408 then
		...
		setTimer ( trashCheckOccus, 60000, 1, veh )
		MtxSetElementData ( veh, "TrashDriverVehicle", getPlayerName(player) )
		...
		MtxSetElementData ( player, "isWorking", true )
...
function trashVehDestroy_func ( veh )
	...
	if isElement ( veh ) then
		...
		if model == 574 or model == 408 then
			...
			MtxSetElementData ( player, "isWorking", false )
