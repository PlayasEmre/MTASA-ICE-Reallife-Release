...
addEventHandler("onBankraubKlingelStart", getRootElement(), function()
	...
	sound = playSound3D(":"..getResourceName(getThisResource()).."/sounds/klingel.mp3", 358.22827148438, 161.99220275879, 1027.7963867188, true)
...
local function setPedAttackPlayer(thePed)
	...
	attackTimer[thePed] = setTimer(function()
		if(getElementHealth(thePed) < 1) then
			if isTimer ( attackTimer[thePed] ) then killTimer ( attackTimer[thePed] ) end
		else
