...
function hotdogBuyMarkerHit ( hit )
	if getElementType ( hit ) == "player" then
		...
		if veh then
			if getElementModel ( hit ) ~= 588 and getPedOccupiedVehicleSeat ( hit ) == 0 then
...
function hotdogjobiconHit ( player )
	if MtxGetElementData ( player, "job" ) == "hotdog" then
...
function hotdogVehicleEnter ( player, seat )
	if MtxGetElementData ( player, "job" ) ~= "hotdog" and seat == 0 then
	...
	if player == client or not client then
		if amount then
			if target then
				...
				target = getPlayerFromName ( MtxGetElementData ( player, "curclicked" ) )
			end
			if amount and isElement ( target ) then
				local amount = math.abs ( math.floor ( tonumber ( amount ) ) )
				outputChatBox ( getPlayerName ( player ).." hat dir einen Hotdog für "..amount.." "..Tables.waehrung.." angeboten! Tippe /accepthotdog, um anzunehmen!", target, 125, 125, 200 )
				MtxSetElementData ( target, "hotdogSeller", getPlayerName ( player ) )
				MtxSetElementData ( target, "hotdogPrice", amount )
...
function accepthotdog_func ( player )
	local seller = getPlayerFromName ( MtxGetElementData ( player, "hotdogSeller" ) )
	local price = MtxGetElementData ( player, "hotdogPrice" )
			...
			if hotdogsInVehicle[veh] then
				if hotdogsInVehicle[veh] >= 1 then
					if MtxGetElementData ( player, "money" ) >= price then
						...
						outputChatBox ( getPlayerName ( player ).." hat einen deiner Hotdogs gekauft - du erhälst "..price.." "..Tables.waehrung.." und hast noch "..hotdogsInVehicle[veh] .." weitere Hotdogs!", seller, 15, 125, 15 )
	...
	MtxSetElementData ( player, "hotdogSeller", nil )
	MtxSetElementData ( player, "hotdogPrice", nil )
end
...
function buyhotdogs_func ( player, amount )
	if player == client then
		...
		local preis = amount
		if veh then
			if MtxGetElementData ( player, "money" ) >= preis then
