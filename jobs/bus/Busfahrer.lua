...
function dontStealTheBus ( hitter, seat, jacked )
...
function BusStart ( hitter, seat, jacked )
	if seat == 0 then
		local hitterjob = MtxGetElementData ( hitter, "job" )
		if (hitterjob == "busfahrer") then
			...
			setElementID ( createBlip ( -1996.92578125,490.61520385742,35.015625, 0, 2, 255, 0, 0, 255, 0, 99999.0, hitter ), "blip" .. getPlayerName(hitter) )
			triggerClientEvent ( hitter, "infobox_start", getRootElement(), "Fahre zur\nHaltestelle!", 7500, 0, 125, 0 )
			addEventHandler( "onMarkerHit", marker, finishbus )
			addEventHandler ( "onPlayerQuit", hitter, quitPlayer )
			onBus[hitter] = true
		else
			removePedFromVehicle(hitter)
...
bus1 = createVehicle ( 437, -2268.3391113281, 185.24940490723,35.1640625, 0,0,90, "BUS" )
...
bus2 = createVehicle ( 437, -2268.3391113281, 192.70036315918,35.1640625, 0,0,90, "BUS" )
...
bus3 = createVehicle ( 437, -2268.3391113281, 200.82695007324,35.1640625, 0,0,90, "BUS" )
...
bus4 = createVehicle ( 437, -2268.3391113281, 208.45877075195,35.1640625, 0,0,90, "BUS" )
...
bus5 = createVehicle ( 437, -2268.3391113281, 216.10036315918,35.1640625, 0,0,90, "BUS" )
...
function finishbus ( hitter, matchingDimension )
	if getElementType ( hitter ) == "vehicle" then
		if getElementModel(hitter) == 437 then
...
function quitPlayer ( quitType )
	if isPedInVehicle(source) then
		...
		if veh and getElementModel(veh) == 437 and getPedOccupiedVehicleSeat(source) == 0 then
			respawnVehicle ( veh )
