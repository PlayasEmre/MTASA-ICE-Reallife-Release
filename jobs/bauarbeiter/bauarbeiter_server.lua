...
jobicons["Bauarbeiter"] = createPickup ( 820.48828125,857.15643310547,12.005271911621, 3, 1239, 250 )
local blip = createBlip ( 820.48828125,857.15643310547,12.005271911621, 58, 2, 255, 255, 0, 255, 0, 200 )
local lastTimerMarkerHit = {}
local JobCounter = {}

addEventHandler ( "onPickupHit", jobicons["Bauarbeiter"],
	function ( player )
		if not MtxGetElementData ( player, "isInBauingJob" ) and not getPedOccupiedVehicle ( player ) then
			if MtxGetElementData ( player, "job" ) == "bauarbeiter" then
...
function BauarbeiterJobRecieve ( action )
	...
	local inBauingJob = MtxGetElementData ( player, "isInBauingJob" ) == true
	...
	if action == "skin" then
		if MtxGetElementData ( player, "bauarbeiterLVL" ) >= 500 then
			setElementModel ( player, 27 )
			MtxSetElementData ( player, "skinid", 27 )
		else
	...
	elseif action == "job1" then
		if not inBauingJob then
			...
			MtxSetElementData ( player, "isInBauingJob", true )
			
			triggerClientEvent ( player, "startBauingJob", player )
		end
	elseif action == "job2" then
		if not inBauingJob then
			if MtxGetElementData ( player, "bauarbeiterLVL" ) >= 100 then
				if MtxGetElementData ( player, "carlicense" ) == 1 then
					...
					MtxSetElementData ( player, "BauingJobVeh", veh )
	...
	elseif action == "job3" then
		if not inBauingJob then
			if MtxGetElementData ( player, "bauarbeiterLVL" ) >= 250 then
				if MtxGetElementData ( player, "lkwlicense" ) == 1 then
					JobCounter[player] = 0
...
function cancelBauingJob ( player )
	...
	if isElement ( MtxGetElementData ( player, "BauingJobVeh" ) ) then
		destroyElement ( MtxGetElementData ( player, "BauingJobVeh" ) )
	end
	MtxSetElementData ( player, "BauingJobVeh", false )
	MtxSetElementData ( player, "isInBauingJob", false )
