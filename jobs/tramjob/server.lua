...
addEventHandler ( "onPickupHit", tramjobicon, function ( player )
	if MtxGetElementData ( player, "job" ) == "tramfuehrer" then
	...
	if MtxGetElementData ( player, "job" ) == "none" then
		if getDistanceBetweenPoints3D ( x, y, z, xj, yj, zj ) < 5 then
			if MtxGetElementData ( player, "carlicense" ) == 1 then
				MtxSetElementData ( player, "job", "tramfuehrer" )
...
local function tramJobberAway ( )
	...
	if isElement(tramBlipArray[source]) then
...
local function tramLeft ( player )
	...
	if isElement(tramBlipArray[player]) then
