	...
	if getPedOccupiedVehicle ( player ) == false then
		if player == localPlayer then
			if tonumber ( vioClientGetElementData ( "jobtime" ) ) == 0 then
				...
				if rnd == 1 then
					planeDrugRoute()
					drugJobPlaneMarker = createBlip ( -2301.7600097656, -2804.5095214844, 14.660495758057, 0, 2, 255, 0, 0, 255, 0, 99999.0, localPlayer )
				elseif rnd == 2 then
					sfpdDrugRoute()
					curjob = "sfpd"
				elseif rnd == 3 then
...
function sfpdDrugRoute()
	...
	drugSFPDBlip = createBlip ( -1605.7623291016, 711.326171875, 14, 0, 2, 255, 0, 0, 255, 0, 99999.0, localPlayer )
...
function drugsFoundClient ()
	...
	triggerServerEvent ( "drugRecieve", localPlayer, drugsfound )
end
