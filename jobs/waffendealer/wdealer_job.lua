...
function wdealerjobiconHit ( player )
	...
	if MtxGetElementData ( player, "job" ) == "wdealer" then
...
function sellgun_func ( player, cmd, target, ggst, ammo )
	...
	if target and getPlayerFromName ( target ) then
		...
		if tonumber(MtxGetElementData ( target, "gunlicense" )) == 1 then
			...
			if getDistanceBetweenPoints3D ( tx, ty, tz, x, y, z ) < 10 then
				...
				if veh then
					if MtxGetElementData ( veh, "mats" ) then
				...
				if not sellingFromGunTruckVan then
					mats = tonumber ( MtxGetElementData ( player, "mats" ) )
				else
					mats = MtxGetElementData ( veh, "mats" )
				end
				MtxSetElementData ( player, "lastcrime", "mats" )
				if ggst == "9mmsd" then
					if mats >= 10 then
						if sellingFromGunTruckVan then
							MtxSetElementData ( getPedOccupiedVehicle ( player ), "mats", mats - 10 )
						else
							MtxSetElementData ( player, "mats", mats - 10 )
						end
				...
				elseif ggst == "9mm" then
					if mats >= 7 then
						if sellingFromGunTruckVan then
							MtxSetElementData ( getPedOccupiedVehicle ( player ), "mats", mats - 7 )
						else
							MtxSetElementData ( player, "mats", mats - 7 )
						end
						playSoundFrontEnd ( player, 40 )
						playSoundFrontEnd ( target, 40 )
				...
				elseif ggst == "eagle" then
					if mats >= 20 then
						if sellingFromGunTruckVan then
							MtxSetElementData ( getPedOccupiedVehicle ( player ), "mats", mats - 20 )
						else
							MtxSetElementData ( player, "mats", mats - 20 )
						end
						playSoundFrontEnd ( player, 40 )
						playSoundFrontEnd ( target, 40 )
				...
				elseif ggst == "mp5" then
					if mats >= 17 then
						if sellingFromGunTruckVan then
							MtxSetElementData ( getPedOccupiedVehicle ( player ), "mats", mats - 17 )
						else
							MtxSetElementData ( player, "mats", mats - 17 )
						end
						playSoundFrontEnd ( player, 40 )
						playSoundFrontEnd ( target, 40 )
				...
				elseif ggst == "messer" then
					if mats >= 3 then
						if sellingFromGunTruckVan then
							MtxSetElementData ( getPedOccupiedVehicle ( player ), "mats", mats - 3 )
						else
							MtxSetElementData ( player, "mats", mats - 3 )
						end
						playSoundFrontEnd ( player, 40 )
						playSoundFrontEnd ( target, 40 )
				...
				elseif ggst == "ak47" then
					if mats >= 25 then
						if sellingFromGunTruckVan then
							MtxSetElementData ( getPedOccupiedVehicle ( player ), "mats", mats - 25 )
						else
							MtxSetElementData ( player, "mats", mats - 25 )
						end
						playSoundFrontEnd ( player, 40 )
						playSoundFrontEnd ( target, 40 )
				...
				elseif ggst == "mats" and not sellingFromGunTruckVan then
					...
					if mats >= ammo then
						MtxSetElementData ( player, "mats", mats - ammo )
						playSoundFrontEnd ( player, 40 )
						playSoundFrontEnd ( target, 40 )
						
						MtxSetElementData ( target, "mats", tonumber ( MtxGetElementData ( target, "mats" ) ) + ammo )
...
-- Beim Event bestimmt der Client alle Argumente, auch den Verkaeufer - Absender
-- erzwingen, sonst liessen sich fremde Materialien ausgeben.
addEventHandler ( "sellgun", getRootElement(), function ( _, cmd, target, ggst, ammo )
	if not isElement ( client ) then return end
	sellgun_func ( client, cmd, target, ggst, ammo )
end )
		...
		if MtxGetElementData ( player, "money" ) >= 200 then
			if tonumber ( MtxGetElementData ( player, "jobtime" ) ) == 0 then
				...
				if getDistanceBetweenPoints3D ( -2627.5083007813, 209.36631774902, 4.1959328651428, x, y, z ) < 10 then
					MtxSetElementData ( player, "lastcrime", "mats" )
					MtxSetElementData ( player, "mats", tonumber ( MtxGetElementData ( player, "mats" ) ) + 20 )
					MtxSetElementData ( player, "jobtime", 5 )
					MtxSetElementData ( player, "money", MtxGetElementData(player,"money")-200 )
				else
					triggerClientEvent ( player, "infobox_start", getRootElement(), "\n\nDu bist zu\nweit weg!", 7000, 125, 0, 0 )
				end
			else
				triggerClientEvent ( player, "infobox_start", getRootElement(), "\n\nDu musst noch\n"..MtxGetElementData(player,"jobtime").." Minuten warten!", 7000, 125, 0, 0 )
			end
		else
