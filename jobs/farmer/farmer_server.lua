...
addEventHandler ( "onPickupHit", jobicons["farmer"],
	function ( player )
		if not isInFarmJob[player] and not getPedOccupiedVehicle ( player ) then
			if MtxGetElementData ( player, "job" ) == "farmer" then
...
function farmerJobRecieve ( action )
	...
	if action == "skin" then
		if MtxGetElementData ( player, "farmerLVL" ) >= 500 then
			setElementModel ( player, 158 )
			MtxSetElementData ( player, "skinid", 158 )
		else
	...
	elseif action == "job2" then
		if not inFarmJob then
			if MtxGetElementData ( player, "farmerLVL" ) >= 100 then
	...
	elseif action == "job3" then
		if not inFarmJob then
			if MtxGetElementData ( player, "farmerLVL" ) >= 250 then
