...
function truckerJobIconHit_func ( player )
	...
	if MtxGetElementData ( player, "job" ) == "transporteur" and not getPedOccupiedVehicle ( player ) then
...
function startTransporteurRoute_func ( vehicle, time, danger )
	...
	local player = client
	if not MtxGetElementData ( player, "transportStarted" ) then
		MtxSetElementData ( player, "truckerTrailer", false )
		local value = calcTruckerJobTourValue ( vehicle, time, danger )
		MtxSetElementData ( player, "transportStarted", true )
		...
		elseif vehicle == "van" then
			id, z = 440, 15.33
			if MtxGetElementData ( player, "carlicense" ) == 0 then
				infobox ( player, "Du brauchst einen\nFuehrerschein!", 5000, 125, 0, 0 )
				return nil
			end
		else
			if MtxGetElementData ( player, "lkwlicense" ) == 0 then
		...
		MtxSetElementData ( player, "truckerVehicle", vehicle )
		...
		if truck then
			...
			MtxSetElementData ( player, "truckerTrailer", trailer )
			...
			if id == 448 then
				...
				MtxSetElementData ( player, "truckerBlip", blip )
			else
				-- Van
		...
		MtxSetElementData ( vehicle, "TruckerJobShitPlayer", getPlayerName(player) )
		...
		MtxSetElementData ( marker, "value", value )
		MtxSetElementData ( player, "truckerMarker", marker )
	...
	if dim and hit then
		if isElement ( hit ) then
			if getElementType ( hit ) == "player" then
				...
				local marker = MtxGetElementData ( player, "truckerMarker" )
...
function truckerJobVehicleDown ( )
	...
	local veh = MtxGetElementData( source, "TruckerJobShitPlayer" )
...
function addTruckerJobErrorEvents ( player, veh )
	...
	local trailer = MtxGetElementData ( player, "truckerTrailer" )
...
function truckerJobTrailerDetatch ( truck )
	...
	local player = getVehicleOccupant ( truck )
	if isElement ( player ) then
		if MtxGetElementData ( player, "transportStarted" ) then
			cancelTruckerJobForPlayer ( player )
...
function cancelTruckerJobForPlayer ( player )
	...
	local veh = MtxGetElementData ( player, "truckerVehicle" )
	local trailer = MtxGetElementData ( player, "truckerTrailer" )
	local marker = MtxGetElementData ( player, "truckerMarker" )
	local blip = MtxGetElementData ( player, "truckerBlip" )
	
	hideallArrows ( player )
	
	MtxSetElementData ( player, "transportStarted", false )
