...
function showTransporteurRouteSelection_func ()
	...
	local WIN_W, WIN_H = 329, 245
	gWindow["transporteur"] = dgsCreateWindow(screenwidth/2-WIN_W/2,screenheight/2-WIN_H/2,WIN_W,WIN_H,"Transporteur",false, nil, nil, nil, nil, nil, tocolor(16,29,61,255), nil, true)
	dgsWindowSetSizable ( gWindow["transporteur"], false )
	dgsWindowSetMovable ( gWindow["transporteur"], false )
	dgsBringToFront(gWindow["transporteur"])
	...
	dgsRadioButtonSetSelected(gRadio["truckerPizzaboy"],true)
	if skill >= 10 then
	...
	gButton["closeTourButton"] = dgsCreateButton(WIN_W-39-77,152+5,77,40,"Fenster schliessen",false,gWindow["transporteur"],
		nil,nil,nil,nil,nil,nil, tocolor(46,110,255,255), tocolor(123,163,255,255), tocolor(24,74,199,255))
	...
	addEventHandler ( "onDgsMouseClickUp", gButton["startTour"],
		function (btn)
			...
			local timeSelected = isElement ( gCheck["truckerTime"] ) and dgsCheckBoxGetSelected ( gCheck["truckerTime"] ) or false
			local dangerSelected = isElement ( gCheck["truckerDanger"] ) and dgsCheckBoxGetSelected ( gCheck["truckerDanger"] ) or false

			if  dgsRadioButtonGetSelected ( gRadio["truckerPizzaboy"] ) then
				triggerServerEvent ( "startTransporteurRoute", lp, "pizza", timeSelected, dangerSelected )

			elseif  dgsRadioButtonGetSelected ( gRadio["truckerVan"] ) then
				triggerServerEvent ( "startTransporteurRoute", lp, "van", timeSelected, dangerSelected )
			elseif   dgsRadioButtonGetSelected ( gRadio["truckerRoadtrain"] ) then
				triggerServerEvent ( "startTransporteurRoute", lp, "roadtrain", timeSelected, dangerSelected )
			elseif   dgsRadioButtonGetSelected ( gRadio["truckerLinerunner"] ) then
				triggerServerEvent ( "startTransporteurRoute", lp, "linerunner", timeSelected, dangerSelected )
			elseif  dgsRadioButtonGetSelected ( gRadio["truckerTanker"] ) then
				triggerServerEvent ( "startTransporteurRoute", lp, "tanker", timeSelected, dangerSelected )
			end
			if isElement ( gWindow["transporteur"] ) then
				destroyElement ( gWindow["transporteur"] )
			end
...
function refreshTruckerMoneyAmount ()
	...
	if isElement ( gWindow["transporteur"] ) then
		...
		local timeSelected = isElement ( gCheck["truckerTime"] ) and dgsCheckBoxGetSelected ( gCheck["truckerTime"] ) or false
		local dangerSelected = isElement ( gCheck["truckerDanger"] ) and dgsCheckBoxGetSelected ( gCheck["truckerDanger"] ) or false
		local amount = calcTruckerJobTourValue ( veh, timeSelected, dangerSelected )
...
function showTruckerJobDamageBar_func ()
	...
	dgsWindowSetSizable ( gWindow["vehDamage"], false )
	dgsWindowSetMovable ( gWindow["vehDamage"], false )
	dgsBringToFront(gWindow["vehDamage"])
