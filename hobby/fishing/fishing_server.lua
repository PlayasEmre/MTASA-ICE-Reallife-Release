...
function setFishingValues ( player )
	...
	MtxSetElementData ( player, "fishingPole", ( pole == 1 ) )
	MtxSetElementData ( player, "fishingHooks", hooks )
	MtxSetElementData ( player, "fishingWorms", worms )
	
	MtxSetElementData ( player, "fishingFishATyp", fishATyp	)
	MtxSetElementData ( player, "fishingFishBTyp", fishBTyp )
	MtxSetElementData ( player, "fishingFishCTyp", fishCTyp )
	
	MtxSetElementData ( player, "fishingFishAWeight", fishAWeight )
	MtxSetElementData ( player, "fishingFishBWeight", fishBWeight )
	MtxSetElementData ( player, "fishingFishCWeight", fishCWeight )
end
function saveFishingValues ( player )
	...
	local string
	if MtxGetElementData ( player, "fishingPole" ) then
		string = "1|"
	else
		string = "0|"
	end
	string = string..MtxGetElementData ( player, "fishingHooks" ).."|"..MtxGetElementData ( player, "fishingWorms" ).."|"
	string = string..MtxGetElementData ( player, "fishingFishATyp" )..";"..MtxGetElementData ( player, "fishingFishAWeight" ).."|"
...
function fishCought_func ()
	...
	local player = client
	if MtxGetElementData ( player, "fishingPole" ) and MtxGetElementData ( player, "fishingHooks" ) > 0 and MtxGetElementData ( player, "fishingWorms" ) > 0 then
		if getFreeFishSlot ( player ) then
			...
			if math.random ( 1, math.floor ( skill ^ ( 0.5 ) * 10 ) ) >= 5 then
				...
				if fishTyp == 7 then
					MtxSetElementData ( player, "money", MtxGetElementData ( player, "money" ) + weight )
					infobox ( player, "Du hast einen\nGeldbeutel gefangen!\nEs sind "..weight.." $ drin!", 5000, 0, 125, 0 )
				else
					MtxSetElementData ( player, "fishingFish"..slot.."Typ", fishTyp )
					MtxSetElementData ( player, "fishingFish"..slot.."Weight", weight )
					infobox ( player, "Du hast etwas\ngefangen:\n"..fishNames[fishTyp], 5000, 0, 200, 0 )
				end
			else
				infobox ( player, "Die Schnur ist\ngerissen!", 5000, 125, 0, 0 )
			end
			MtxSetElementData ( player, "fishingHooks", MtxGetElementData ( player, "fishingHooks" ) - 1 )
			MtxSetElementData ( player, "fishingWorms", MtxGetElementData ( player, "fishingWorms" ) - 1 )
		else
...
function getFreeFishSlot ( player )
	...
	if MtxGetElementData ( player, "fishingFishATyp" ) == 0 then
		return "A"
	elseif MtxGetElementData ( player, "fishingFishBTyp" ) == 0 then
		return "B"
	elseif MtxGetElementData ( player, "fishingFishCTyp" ) == 0 then
		return "C"
	end
	return false
end
...
function fishShopBuy_func ( item, amount )
	...
	if item == "pole" then
		if money >= fishingPolePrice then
			if not MtxGetElementData ( player, "fishingPole" ) then
				MtxSetElementData ( player, "money", money - fishingPolePrice )
				infobox ( player, "Du hast eine\nAngel gekauft!\nDie Angel ist\njetzt in deinem\nInventar.", 5000, 0, 125, 0 )
				MtxSetElementData ( player, "fishingPole", true )
			else
		...
		if MtxGetElementData ( player, "fishingPole" ) then
			...
			if item == "hooks" then
				local cur = MtxGetElementData ( player, "fishingHooks" )
				if money >= fishingHookPrice * amount then
					if cur + amount <= max then
						MtxSetElementData ( player, "money", money - fishingHookPrice * amount )
						infobox ( player, "Du hast "..amount.."\nAngelhaken gekauft!", 5000, 0, 125, 0 )
						MtxSetElementData ( player, "fishingHooks", amount + cur )
					else
				...
				local cur = MtxGetElementData ( player, "fishingWorms" )
				if money >= fishingWormPrice * amount then
					if cur + amount <= max then
						MtxSetElementData ( player, "money", money - fishingWormPrice * amount )
						infobox ( player, "Du hast "..amount.."\nWuermer gekauft!", 5000, 0, 125, 0 )
						MtxSetElementData ( player, "fishingWorms", amount + cur )
					else
	...
	if getDistanceBetweenPoints3D ( x, y, z, getElementPosition ( player ) ) <= 7.5 then
		if tonumber ( slot ) then
			...
			if slot == 1 or slot == 2 or slot == 3 then
				...
				local fish = MtxGetElementData ( player, "fishingFish"..slot.."Typ" )
				if fish then
					local weight = MtxGetElementData ( player, "fishingFish"..slot.."Weight" )
					local money = math.floor ( fishPrices[fish] * weight / 1000 )
					
					MtxSetElementData ( player, "money", MtxGetElementData ( player, "money" ) + money )
					MtxSetElementData ( player, "fishingFish"..slot.."Typ", 0 )
					MtxSetElementData ( player, "fishingFish"..slot.."Weight", 0 )
