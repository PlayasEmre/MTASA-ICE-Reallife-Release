...
function drugsSellHobby_func ( amount )
	...
	if MtxGetElementData ( player, "club" ) == "gartenverein" then
		...
		if drugs >= amount then
			...
			MtxSetElementData ( player, "money", MtxGetElementData ( player, "money" ) + amount * 12 )
...
function addWeed ( id, x, y, z, time, name )
	...
	MtxSetElementData ( weedPlants[id], "id", id )
	MtxSetElementData ( boxA, "id", id )
	MtxSetElementData ( boxB, "id", id )
	
	MtxSetElementData ( weedPlants[id], "time", time )
	MtxSetElementData ( boxA, "time", time )
	MtxSetElementData ( boxB, "time", time )

	-- "time" steht in notSyncedData (core/data.lua) und erreicht den Client
	-- deshalb nie. Fuer die Fortschrittsanzeige ueber der Pflanze wird die
	-- Pflanzzeit hier zusaetzlich unter einem eigenen, synchronisierten
	-- Schluessel abgelegt - nur an der Pflanze selbst, nicht an den Kisten,
	-- damit pro Pflanze genau ein Text erscheint.
	setElementData ( weedPlants[id], "weedPlantTime", time )
	
	MtxSetElementData ( weedPlants[id], "weed", true )
	MtxSetElementData ( boxA, "weed", true )
	MtxSetElementData ( boxB, "weed", true )
...
function grow_func ( player, cmd, planttype )
	...
	if planttype == "weed" then
		if isPedOnGround ( player ) and not getPedOccupiedVehicle ( player ) and getElementInterior ( player ) == 0 and getElementDimension ( player ) == 0 then
			if not MtxGetElementData ( player, "growing" ) then
				if MtxGetElementData ( player, "flowerseeds" ) >= 1 then
					local x, y, z = getElementPosition ( player )
					if z > -5 then
						MtxSetElementData ( player, "growing", true )
						...
						MtxSetElementData ( player, "flowerseeds", MtxGetElementData ( player, "flowerseeds" ) - 1 )
...
function joinGartenverein_func ()
	...
	if money >= 200 then
		MtxSetElementData ( player, "money", money - 200 )
		MtxSetElementData ( player, "club", "gartenverein" )
...
function BuyShovelServer_func ()
	...
	if MtxGetElementData ( player, "club" ) == "gartenverein" then
		local money = tonumber ( MtxGetElementData ( player, "money" ) )
		if money >= 15 then
			MtxSetElementData ( player, "money", money - 15 )
			local weapon = 6
			local ammo = 1
...
function BuyFlowersServer_func ()
	...
	if MtxGetElementData ( player, "club" ) == "gartenverein" then
		...
		if money >= 500 then
			MtxSetElementData ( player, "money", money - 500 )
			
			MtxSetElementData ( player, "flowerseeds", seeds + 10 )
...
addEventHandler ( "leaveGardenclub", getRootElement(),
	function ()
		MtxSetElementData ( client, "club", "none" )
