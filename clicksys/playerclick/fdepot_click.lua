	...
	if gWindow["gangDepot"] then
		...
		addEventHandler("onClientGUIClick", gButton["DepotPut"],
			function ()
				...
				triggerServerEvent ( "fDepotServer", getRootElement(), localPlayer, false, money, drugs, mats )
			end
		)
		addEventHandler("onClientGUIClick", gButton["DepotTake"],
			function ()
				...
				local mats = guiGetText ( gEdit["matsDepot"] )
				local drugs = guiGetText ( gEdit["drugsDepot"] )
				triggerServerEvent ( "fDepotServer", getRootElement(), localPlayer, true, money, drugs, mats )
			end
		)
		...
		gLabel["moneyValue"] = guiCreateLabel(0.0444,0.2977,0.3389,0.1069,"0 "..Tables.waehrung.."",true,gWindow["gangDepot"])
...
function setDepotEntrys ( fmoney, fmats, fdrugs )
	...
	guiSetText ( gLabel["moneyValue"], fmoney.." "..Tables.waehrung.."" )
