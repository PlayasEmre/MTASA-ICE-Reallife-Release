...
MtxSetElementData ( PnsSFWangCars, "gateID", 19 )
MtxSetElementData ( PnsLVCity, "gateID", 36 )
MtxSetElementData ( PnsFortCarson, "gateID", 41 )
MtxSetElementData ( PnsIdleWood, "gateID", 8 )
MtxSetElementData ( PnsDillimore, "gateID", 47 )
MtxSetElementData ( PnsTemple, "gateID", 11 )
MtxSetElementData ( PnsBeach, "gateID", 12 )
MtxSetElementData ( PnsSFJuniperHill, "gateID", 27 )
...
function paynSprayRepair ( hitElement )
	if hitElement and isElement ( hitElement ) and getElementType ( hitElement ) == "vehicle" then
		if getVehicleOccupant ( hitElement, 0 ) then
			...
			if MtxGetElementData ( player, "money" ) >= paynsprayprice then
				MtxSetElementData ( player, "money", MtxGetElementData ( player, "money" ) - paynsprayprice )
				...
				if (source == PnsSFWangCars and z > 37 and z < 46) or source ~= PnsSFWangCars then
					if getVehicleType ( hitElement ) ~= "Plane" and getVehicleType ( hitElement ) ~= "Helicopter" and getVehicleType ( hitElement ) ~= "Boat" and getVehicleType ( hitElement ) ~= "Train" and getVehicleType ( hitElement ) ~= "Trailer" then
						...
						if isGarageOpen ( MtxGetElementData ( source, "gateID" ) ) == true then
							setGarageOpen ( MtxGetElementData ( source, "gateID" ), false )
						end
						setTimer ( repairthiscarinpaynspray, 3000, 1, hitElement, player, MtxGetElementData ( source, "gateID" ) )
					end									
				end
			else
