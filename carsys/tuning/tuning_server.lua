...
MtxSetElementData ( MarkerTuning1, "sx", -1936.6840820313 )
MtxSetElementData ( MarkerTuning1, "sy", 220.6498260498 )
MtxSetElementData ( MarkerTuning1, "sz", 34.3125 )
MtxSetElementData ( MarkerTuning1, "sr", 0 )

MtxSetElementData ( MarkerTuning2, "sx", 2393.8520507813 )
MtxSetElementData ( MarkerTuning2, "sy", 989.70678710938 )
MtxSetElementData ( MarkerTuning2, "sz", 10.690312385559 )
MtxSetElementData ( MarkerTuning2, "sr", 0 )
	...
	if getElementType( hitElement ) == "vehicle" and matchingDimension then
		if getVehicleOccupant ( hitElement, 0 ) ~= false and getVehicleOccupant ( hitElement, 1 ) == false and getVehicleOccupant ( hitElement, 2 ) == false and getVehicleOccupant ( hitElement, 3 ) == false then
			if not copvehs[getElementModel ( hitElement )] then
				...
				if player then
					if MtxGetElementData ( hitElement, "owner" ) then
						if MtxGetElementData ( hitElement, "owner" ) == getPlayerName ( player ) then
							local x, y, z, r = MtxGetElementData ( source, "sx" ), MtxGetElementData ( source, "sy" ), MtxGetElementData ( source, "sz" ), MtxGetElementData ( source, "sr" )
							
							MtxSetElementData ( hitElement, "tuningSx", x )
							MtxSetElementData ( hitElement, "tuningSy", y )
							MtxSetElementData ( hitElement, "tuningSz", z )
							MtxSetElementData ( hitElement, "tuningSr", r )
							...
							i = tonumber ( MtxGetElementData ( player, "playerid" ) )
							MtxSetElementData ( hitElement, "usingdim", i )
...
function addSpecialTuning_func ( tuning )
	...
	if player == client then
		...
		if money >= price then
			...
			for i = 1, 6 do
				if i == tuning then
					MtxSetElementData ( veh, "stuning"..i, tuning )
					totTuning = totTuning.."1".."|"
					if i == 1 then
						outputChatBox ( "Dein Fahrzeug hat nun einen Kofferraum - benutze die Spezialmissionen-Taste, um ihn zu öffnen.", player, 0, 125, 0 )
						outputChatBox ( "Anschließend kannst du ihn per Clicksystem bedienen und bestimmte Items darin lagern.", player, 0, 125, 0 )
					elseif i == 2 then
					...
					local tok = gettok ( MtxGetElementData ( veh, "stuning" ), i, string.byte ( '|' ) )
...
function CancelTuning_func ( player, veh, c1, c2, c3, c4, paintjob, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16 )
	...
	if player == client then
		...
		MtxSetElementData ( veh, "color", color )
		...
		local x, y, z, r = MtxGetElementData ( veh, "tuningSx" ), MtxGetElementData ( veh, "tuningSy" ), MtxGetElementData ( veh, "tuningSz" ), MtxGetElementData ( veh, "tuningSr" )
