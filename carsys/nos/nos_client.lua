...
function useNosCheck ( key, state )
	...
	elseif nosInUsage and state ~= "down" and key == nosKey then
		syncNosWithServer ()
		nosKey = nil
		triggerServerEvent ( "endNosUsage", localPlayer )
		nosInUsage = false
		if isTimer ( nosTimer ) then killTimer ( nosTimer ) end
	end
end
...
addEventHandler ( "onClientPlayerVehicleExit", localPlayer, vehicleWithNosExit_func )
...
addEventHandler ( "onClientPlayerVehicleEnter", localPlayer, vehicleWithNosEnter_func )
...
function nosUsage ()
	...
	if veh then
		if usedNos >= getElementData ( veh, "nitro" ) or ( getElementData ( veh, "nitro" ) - usedNos ) < 0 then
			...
			triggerServerEvent ( "endNosUsage", localPlayer )
			nosKey = nil
			triggerServerEvent ( "removeNosFromVehicle", localPlayer )
			nosInUsage = false
			if isTimer ( nosTimer ) then killTimer ( nosTimer ) end
		end
	end
