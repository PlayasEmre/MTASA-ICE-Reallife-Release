...
function mySQLCarhouseCreate ( array )
	for i=1, #array do
		...
		MtxSetElementData ( MySQLCarhouses[ID]["pickup"], "ID", ID )
		
		addEventHandler ( "onPickupHit", MySQLCarhouses[ID]["pickup"], function ( player )
			if getElementType ( player ) == "player" then
				if not MtxGetElementData ( player, "isInCarHouse" ) then
					if not getPedOccupiedVehicle ( player ) then
						MtxSetElementData ( player, "isInCarHouse", true )
						MySQLCarhouseEnter ( player, MtxGetElementData ( source, "ID" ) )
					end
				end
			end
		end )
...
function MySQLCarhouseEnter ( player, ID )
	...
	MtxSetElementData ( player, "lookingAtCar", 1 )
	MtxSetElementData ( player, "carHouse", ID )
...
function MySQLCarhouseLeft ( player )
	...
	local car = MtxGetElementData ( player, "lookingAtCar" )
	local id = MtxGetElementData ( player, "carHouse" )
	local count = MySQLCarhouses[id]["counter"]

	if car == 1 then
		MtxSetElementData ( player, "lookingAtCar", count )
	else
		MtxSetElementData ( player, "lookingAtCar", MtxGetElementData ( player, "lookingAtCar" ) - 1 )
	end
...
function leaveMySQLCarhouseCar ( player )
	...
	MtxSetElementData ( player, "isInCarHouse", false )
