...
function playerDropFromHeli(vehicle, reason, forceDrop, inVehicle)
	...
	if client and isElement(client) then
		...
		-- Empfaenger sammeln und in EINEM triggerClientEvent verschicken statt
		-- einer Netzwerkoperation pro Spieler. MTA nimmt dafuer eine Tabelle
		-- entgegen; Event, Quelle und Argumente bleiben identisch.
		local empfaenger = {}
		local players = getElementsByType("player")
		for i=1, #players do
			if (forceDrop) or (players[i] ~= client) then
				empfaenger[#empfaenger+1] = players[i]
			end
		end
		if #empfaenger > 0 then
			triggerClientEvent(empfaenger, "stopPlayerGrab", client, vehicle, nil, reason, false)
		end
