...
function buySpezialLack_func(red1, green1, blue1, red2, green2, blue2)
	...
	if veh then
		...
		if MtxGetElementData(veh, "spezcolor") == "" then
			...
			if MtxGetElementData(player, "money") >= 1250 then
				MtxSetElementData(player, "money", MtxGetElementData(player, "money") - 1250)
			else
				triggerClientEvent ( player, "infobox_start", getRootElement(), "\nDu hast nicht\ngenug Geld!", 5000, 255, 0, 0 )
				return false
			end
...
function deleteSpezialLack_func(player)
	...
	if veh then
		...
		if getDistanceBetweenPoints3D ( -1782.215, 1205.233, 25, x, y, z ) <= 5 then
			if MtxGetElementData ( veh, "owner" ) == getPlayerName ( player ) then
				local colors = MtxGetElementData ( veh, "color" )
