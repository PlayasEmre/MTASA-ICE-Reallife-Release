...
function pickupPackage ( player )
	...
	local number = MtxGetElementData ( source, "pickupID" )
	if number then
		if tonumber ( MtxGetElementData ( player, "package"..number ) ) == 0 then
			local pname = getPlayerName ( player )
			MtxSetElementData ( player, "package"..number, 1 )
			MtxSetElementData ( player, "foundpackages", MtxGetElementData ( player, "foundpackages" ) + 1 )
			outputChatBox ( "Du hast "..MtxGetElementData ( player, "foundpackages" ).." von 25 versteckten Päckchen gefunden und erhaelst 10 Punkte sowie 100 "..Tables.waehrung.."!", player, 0, 125, 0 )
...
for i = 1, #packages do
	...
	MtxSetElementData ( packages[i], "pickupID", i )
end


function PackageAchievCheck ( player )

	if MtxGetElementData ( player, "collectr_achiev" ) and MtxGetElementData ( player, "foundpackages" ) then
		if MtxGetElementData ( player, "collectr_achiev" ) ~= "done" and tonumber(MtxGetElementData ( player, "foundpackages" )) >= 25 then						-- Achiev: Collector
